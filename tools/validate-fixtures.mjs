...
import {
  ...
  leafOf,
  reasonMatches,
} from "./check-fixture-reasons.mjs";
import {
  AUTHORING_SCHEMA_PATH,
  SCHEMA_PATH,
  schemaCompiler,
} from "./compile-schema.mjs";
...
for (const file of jsonFiles(join(root, "fixtures", "valid"))) {
  ...
  validate(doc)
    ...
    : fail(relative(root, file), errorText());
}

console.log(
  "\nfixtures/invalid — every document must be rejected, for its stated reason",
);
const declaredReason = declaredReasons(
  readFileSync(join(root, "fixtures", "README.md"), "utf8"),
);
for (const file of jsonFiles(join(root, "fixtures", "invalid"))) {
  ...
  if (declared === undefined) {
    ...
    const got = [
      ...new Set(validate.errors.map((e) => `${e.keyword}:${leafOf(e)}`)),
    ];
...
const recipeFiles = jsonFiles(recipesDir).filter(
  (f) => basename(f) !== "catalog.json",
);
...
const catalog = JSON.parse(
  readFileSync(join(recipesDir, "catalog.json"), "utf8"),
);
const catalogSlugs = new Set(catalog.recipes.map((r) => r.slug));
const fileSlugs = new Set(recipeFiles.map((f) => basename(f, ".json")));
for (const slug of catalogSlugs)
  fileSlugs.has(slug)
    ? ok(`catalog → ${slug}.json`)
    : fail(`catalog → ${slug}.json`, "no such file");
for (const slug of fileSlugs)
  catalogSlugs.has(slug)
    ? ok(`${slug}.json → catalog`)
    : fail(`${slug}.json → catalog`, "no catalog entry");
...
const firstMagnitude = (m) => {
  ...
  for (const key of ["value", "max", "min"])
    if (typeof m[key] === "number") return m[key];
  return null;
};
const scheduleBreaks = (doc) => {
  const breaks = [];
  for (const [r, recipe] of (Array.isArray(doc.recipes)
    ? doc.recipes
    : []
  ).entries()) {
    const last = { at_s: null, to_water: null };
    for (const [i, step] of (Array.isArray(recipe.steps)
      ? recipe.steps
      : []
    ).entries()) {
      if (typeof step !== "object" || step === null) continue;
      const seen = {
        at_s: typeof step.at_s === "number" ? step.at_s : null,
        to_water: firstMagnitude(step.to_water),
      };
      for (const field of ["at_s", "to_water"]) {
        if (seen[field] === null) continue;
        if (last[field] !== null && seen[field] < last[field])
          breaks.push(
            `recipes[${r}].steps[${i}] ${field} ${seen[field]} follows ${last[field]}`,
          );
...
for (const file of [
  ...recipeFiles,
  ...jsonFiles(join(root, "fixtures", "valid")),
]) {
  const breaks = scheduleBreaks(JSON.parse(readFileSync(file, "utf8")));
  breaks.length === 0
    ? ok(`schedule ${relative(root, file)}`)
    : fail(`schedule ${relative(root, file)}`, breaks.join("\n"));
}
...
const { renderAuthoringSchema, buildAuthoringSchema } = await import(
  "./gen-authoring-schema.mjs"
);
...
readFileSync(AUTHORING_SCHEMA_PATH, "utf8") === renderAuthoringSchema(runtime)
  ...
  : fail(
      "authoring schema matches its generator (no drift)",
      "regenerate: node tools/gen-authoring-schema.mjs",
    );
...
renderGearLabels(
  JSON.parse(readFileSync(join(root, "registries/gear.json"), "utf8")).gear,
).out
  ? ok("bundled gear labels match the registry (no drift)")
  : fail(
      "bundled gear labels match the registry (no drift)",
      "regenerate: node tools/gen-gear-labels.mjs",
    );
...
const authoringExempt = new Map([
  ...
  [
    "forward-compat-unknown-registry-values.json",
    "unknown registry values and unknown members are its purpose",
  ],
...
for (const file of [
  ...
  if (authoringExempt.has(name)) {
    console.log(
      `  --  ${relative(root, file)} (exempt: ${authoringExempt.get(name)})`,
    );
    continue;
  }
  const doc = JSON.parse(readFileSync(file, "utf8"));
...
validateAuthoring(typoProbe)
  ? fail(
      "typo probe rejected by the authoring schema",
      "validated, but watter_temp must be caught",
    )
...
validate(vendorTypoProbe)
  ? ok(
      "a typo beside the vendor member stays valid at runtime (objects are open)",
    )
  : fail(
      "a typo beside the vendor member stays valid at runtime (objects are open)",
      errorText(),
...
validateAuthoring(vendorTypoProbe)
  ? fail(
      "authoring still catches a typo beside the vendor member",
      "validated, but roaster_note must be caught",
    )
  ...
  recipes: [
    {
      ...
      localizations: {
        en: { title: "4:6 Method", coffee: { value: 18, unit: "gram" } },
      },
    },
  ],
};
validate(localizedDose)
  ? ok(
      "a localization carrying a quantity stays valid at runtime (objects are open)",
    )
  : fail(
      "a localization carrying a quantity stays valid at runtime (objects are open)",
      errorText(),
...
validateAuthoring(localizedDose)
  ? fail(
      "authoring refuses a quantity inside a localization",
      "validated, but a per-locale dose is a different recipe",
    )
  ...
  recipes: [
    {
      ...
      water: { value: 300, unit: "gram" },
      localizations: {
        en: { title: "4:6 Method", ext: { "app.example": { tone: "casual" } } },
      },
    },
  ],
};
validate(localizedExt)
  ? ok(
      "the reserved vendor member inside a localization stays valid at runtime",
    )
  : fail(
      "the reserved vendor member inside a localization stays valid at runtime",
      errorText(),
...
validateAuthoring(localizedExt)
  ? fail(
      "authoring refuses the reserved vendor member inside a localization",
      "validated, but a localization carries wording only",
    )
...
validate(unlinkedInMultiBean)
  ...
  : fail(
      "an unreferenced recipe in a multi-bean document stays valid at runtime",
      errorText(),
    );
validateAuthoring(unlinkedInMultiBean)
  ? fail(
      "authoring flags an unreferenced recipe once a document carries several beans",
      "validated, but co-location no longer associates anything here",
    )
  : ok(
      "authoring flags an unreferenced recipe once a document carries several beans",
    );
...
const referencedInMultiBean = {
  coffeejson: "1.0",
  beans: [bean("a"), bean("b")],
  recipes: [recipe({ bean_ref: "a" })],
};
validateAuthoring(referencedInMultiBean)
  ? ok("authoring accepts a referenced recipe in a multi-bean document")
  : fail(
      "authoring accepts a referenced recipe in a multi-bean document",
      (validateAuthoring.errors ?? [])
        .map((e) => `${e.instancePath || "/"} ${e.message}`)
        .join("\n"),
    );
...
validateAuthoring(bagToBrew)
  ? ok(
      "authoring leaves bag-to-brew alone — one bean still associates by co-location",
    )
  : fail(
      "authoring leaves bag-to-brew alone — one bean still associates by co-location",
      (validateAuthoring.errors ?? [])
...
const specDoc = (name) =>
  readFileSync(join(root, "docs", "spec", name), "utf8");
const proseDocs = {
...
const probes = [
  [
    "a schema field the prose lacks is caught",
    (s) => {
      s.$defs.recipe.properties.ghost_field = { type: "string" };
    },
    null,
    (f) => f.label === "fields recipe" && f.error?.includes("ghost_field"),
  ],
  [
    "a schema enum value the prose still lists is caught",
    (s) => {
      s.$defs.method.enum = s.$defs.method.enum.filter((v) => v !== "moka");
    },
    null,
    (f) => f.label === "vocab `method`" && f.error?.includes("moka"),
  ],
  [
    "a renamed prose section fails loudly",
    null,
    (d) => ({
      ...d,
      recipe: d.recipe.replace("## Gear object", "## Gear thing"),
    }),
    (f) => f.label === "fields gear" && f.error?.includes("not found"),
  ],
  [
    "a schema field the tasting prose lacks is caught",
    (s) => {
      s.$defs.tasting.properties.ghost_field = { type: "string" };
    },
    null,
    (f) => f.label === "fields tasting" && f.error?.includes("ghost_field"),
  ],
  [
    "a perceived dimension the prose lacks is caught",
    (s) => {
      s.$defs.tasting.properties.perceived.properties.body = { type: "number" };
    },
    null,
    (f) => f.label === "fields perceived" && f.error?.includes("body"),
  ],
  [
    "a requiredness disagreement is caught",
    (s) => {
      s.$defs.party.required = [];
    },
    null,
    (f) => f.label === "required party.name" && f.error,
  ],
  [
    "a Type-cell disagreement is caught",
    null,
    (d) => ({
      ...d,
      bean: d.bean.replace(
        "| `process` | array of string (enum) |",
        "| `process` | string (enum) |",
      ),
    }),
    (f) =>
      f.label === "type bean.process" && f.error?.includes("schema says array"),
  ],
];
...
coverageFindings(ghosted, { ts: coreTypes }).some((f) =>
  f.error?.includes("tasting.ghost_key"),
)
  ? ok("coverage probe: a schema key no type names is caught")
  : fail(
      "coverage probe: a schema key no type names is caught",
      "seeded key produced no finding",
    );
const orphaned = structuredClone(runtime);
orphaned.$defs.ghost_object = {
  type: "object",
  properties: { a: { type: "string" } },
};
coverageFindings(orphaned, { ts: coreTypes }).some((f) =>
  f.error?.includes("ghost_object.a"),
)
  ? ok("coverage probe: a new $def nobody mapped is reported, not skipped")
  : fail(
      "coverage probe: a new $def nobody mapped is reported, not skipped",
      "seeded $def produced no finding",
    );
// A key must be named on ITS OWN type, not merely appear somewhere in the file.
coverageFindings(runtime, {
  ts: coreTypes.replace(/export interface Bean\b/, "export interface NotBean"),
}).some((f) => f.error?.includes("bean.name"))
  ? ok(
      "coverage probe: a key named only on some other type does not count as read",
    )
  : fail(
      "coverage probe: a key named only on some other type does not count as read",
      "seeded rename produced no finding",
    );

console.log("\ndocumentation links — every relative link and #anchor resolves");
const { linkFindings, anchorsOf, servedRepoPath } = await import(
  "./check-doc-links.mjs"
);
...
linkFindings(seededDead).some((f) => f.error === "no heading with that anchor")
  ...
  : fail(
      "links probe: a dead anchor is caught",
      "seeded dead anchor produced no finding",
    );
linkFindings(new Map([["a.md", "[x](gone.md)"]])).some(
  (f) => f.error === "unknown target",
)
  ? ok("links probe: a missing target is caught")
  : fail(
      "links probe: a missing target is caught",
      "seeded missing file produced no finding",
    );
...
linkFindings(
  new Map([["a.md", "[x](https://coffeejson.org/docs/gone.md)"]]),
).some((f) => f.error === "unknown target")
  ? ok("links probe: a dead canonical-host target is caught")
  : fail(
      "links probe: a dead canonical-host target is caught",
      "seeded missing served file produced no finding",
    );
servedRepoPath("https://example.com/docs/spec/01-overview.md") === null
  ...
  : fail(
      "links probe: another host's path is not read as this repo's",
...
anchorsOf("### Espresso (dose : yield)").has("espresso-dose--yield")
  ? ok(
      "links probe: a double-hyphen anchor is not 'tidied' into a false failure",
    )
  : fail(
      "links probe: a double-hyphen anchor is not 'tidied' into a false failure",
      "slug rule diverged from GitHub's",
    );
...
const gearRegistry = JSON.parse(
  readFileSync(join(root, "registries", "gear.json"), "utf8"),
);
const varietalRegistry = JSON.parse(
  readFileSync(join(root, "registries", "varietals.json"), "utf8"),
);
...
const TOKEN_REGISTRIES = [
  {
    file: "addition-types.json",
    key: "addition_types",
    section: "Addition type",
  },
  {
    file: "producer-roles.json",
    key: "producer_roles",
    section: "Producer role",
  },
].map((r) => ({
  ...r,
  data: JSON.parse(readFileSync(join(root, "registries", r.file), "utf8")),
}));
...
const offRegistryByDesign = new Set([
  "forward-compat-unknown-registry-values.json",
]);
const registryDocs = [
  ...jsonFiles(join(root, "fixtures", "valid")).filter(
    (f) => !offRegistryByDesign.has(basename(f)),
  ),
  ...recipeFiles,
].map((f) => ({
  label: relative(root, f),
  doc: JSON.parse(readFileSync(f, "utf8")),
}));
for (const { label, error } of registryFindings(
  gearRegistry,
  varietalRegistry,
  proseDocs.vocabularies,
  registryDocs,
  TOKEN_REGISTRIES,
))
...
const customEntry = {
  gear: [
    ...gearRegistry.gear,
    { id: "custom", roles: ["brewer"], category: "dripper", label: "x" },
  ],
};
registryFindings(
  customEntry,
  ...
  proseDocs.vocabularies,
  [],
).some((f) => f.label === "gear custom" && f.error?.includes("escape hatch"))
  ? ok("registry probe: a custom registry entry is rejected")
  : fail(
      "registry probe: a custom registry entry is rejected",
      "seeded entry produced no finding",
    );
...
const ghostProse = proseDocs.vocabularies.replace(
  "`hario-v60` · `chemex`",
  "`hario-v60` · `ghost-brewer` · `chemex`",
);
registryFindings(gearRegistry, varietalRegistry, ghostProse, []).some(
  (f) => f.label === "prose gear seeds" && f.error?.includes("ghost-brewer"),
)
  ? ok("registry probe: a prose slug missing from the data is caught")
  : fail(
      "registry probe: a prose slug missing from the data is caught",
      "seeded slug produced no finding",
    );
const probeDoc = (id) => ({
  coffeejson: "1.0",
  recipes: [
...
registryFindings(gearRegistry, varietalRegistry, proseDocs.vocabularies, [
  { label: "probe", doc: probeDoc("not-registered") },
]).some(
  (f) =>
    f.label === "document gear probe" && f.error?.includes("not-registered"),
)
  ? ok("registry probe: an unregistered document gear id is caught")
  : fail(
      "registry probe: an unregistered document gear id is caught",
      "seeded id produced no finding",
    );
// A grinder in a brewer slot: JSON Schema cannot see it, and only the registry knows.
registryFindings(gearRegistry, varietalRegistry, proseDocs.vocabularies, [
  { label: "probe", doc: probeDoc("comandante-c40") },
]).some(
  (f) =>
    f.label === "document gear roles probe" &&
    f.error?.includes("comandante-c40 in brewer"),
)
  ? ok("registry probe: gear used in a slot it does not play is caught")
  : fail(
      "registry probe: gear used in a slot it does not play is caught",
      "seeded misplacement produced no finding",
    );
// A non-array `roles` must report, not throw and abort the sweep.
registryFindings(
  { gear: [{ id: "x", label: "X", roles: 5 }] },
  varietalRegistry,
  proseDocs.vocabularies,
  [],
  ...
  : fail(
      "registry probe: a malformed roles reports rather than throwing",
      "seeded value produced no finding",
    );
// An alias is a synonym this repository's own documents never write.
registryFindings(gearRegistry, varietalRegistry, proseDocs.vocabularies, [
  { label: "probe", doc: probeDoc("sage-bambino") },
]).some(
  (f) => f.label === "document gear probe" && f.error?.includes("sage-bambino"),
)
  ? ok("registry probe: an alias id in one of the repo's documents is caught")
  : fail(
      "registry probe: an alias id in one of the repo's documents is caught",
      "seeded alias produced no finding",
    );
const driftedTokens = TOKEN_REGISTRIES.map((r, i) =>
  i === 0
    ? { ...r, data: { ...r.data, [r.key]: [...r.data[r.key], "ghost-token"] } }
    : r,
);
registryFindings(
  gearRegistry,
  ...
  (f) =>
    f.label === "registry parity addition-types.json" &&
    f.error?.includes("ghost-token"),
)
  ? ok("registry probe: a token the prose does not recommend is caught")
  : fail(
      "registry probe: a token the prose does not recommend is caught",
      "seeded token produced no finding",
    );
...
for (const { label, error } of bareNumberFindings(
...
const fixtureNames = (dir) =>
  jsonFiles(join(root, "fixtures", dir)).map((f) => basename(f));
const fixturesReadme = readFileSync(
  join(root, "fixtures", "README.md"),
  "utf8",
);
const fixtureDirs = {
...
catalogFindings(fixturesReadme, {
  ...fixtureDirs,
  invalid: [...fixtureDirs.invalid, "ghost-fixture.json"],
}).some(
  (f) =>
    f.label === "invalid/ table" && f.error?.includes("ghost-fixture.json"),
)
  ? ok("catalog probe: a fixture with no table row is caught")
  : fail(
      "catalog probe: a fixture with no table row is caught",
      "seeded file produced no finding",
    );
catalogFindings(
  fixturesReadme.replace("| `minimal.json` |", "| `renamed-away.json` |"),
  fixtureDirs,
).some((f) => f.label === "valid/ table" && f.error?.includes("minimal.json"))
  ? ok("catalog probe: a table row naming no file is caught")
  : fail(
      "catalog probe: a table row naming no file is caught",
      "seeded row produced no finding",
    );
...
for (const f of consistencyDocs)
  for (const { label, error } of consistencyFindings(
    relative(root, f),
    JSON.parse(readFileSync(f, "utf8")),
  ))
    error ? fail(label, error) : ok(label);

consistencyFindings("probe", {
  recipes: [
    {
      coffee: { value: 15, unit: "gram" },
      water: { value: 250, unit: "gram" },
  ...
  : fail(
      "consistency probe: a ratio the measurements refute is caught",
      "seeded document produced no finding",
    );
consistencyFindings("probe", {
  recipes: [{ coffee: { min: 30, max: 15, unit: "gram" } }],
}).some((f) => f.error?.includes("empty window"))
  ? ok("consistency probe: an inverted min/max window is caught")
  : fail(
      "consistency probe: an inverted min/max window is caught",
      "seeded document produced no finding",
    );
// `rest_days` is a window with no unit, so a unit-gated sweep would walk straight past it.
consistencyFindings("probe", {
  beans: [{ name: "x", rest_days: { min: 30, max: 14 } }],
}).some((f) => f.error?.includes("empty window"))
  ? ok("consistency probe: an inverted window carrying no unit is caught")
  : fail(
      "consistency probe: an inverted window carrying no unit is caught",
      "seeded document produced no finding",
    );
  ...
  recipes: [
    {
      ...
      water: { min: 657, max: 675, unit: "gram" },
      ratio: 15,
    },
  ],
}).some((f) => f.error?.includes("window's min"))
  ? ok(
      "consistency probe: a coupled window whose low end misses the ratio is caught",
    )
  : fail(
      "consistency probe: a coupled window whose low end misses the ratio is caught",
      "seeded document produced no finding",
...
consistencyFindings("probe", {
  ...
  : fail(
      "consistency probe: a cross-unit mass ratio is still compared",
      "seeded document produced no finding",
    );
...
const decodeShareUrl = (input) => {
  ...
  if (bytes.length > 0 && bytes[0] === 0x7b) {
    if (bytes.length > 8192)
      return { rejected: "decoded payload exceeds the 8192-byte cap" };
    payload = bytes;
  } else if (zlibHeader) {
    ...
    return {
      rejected:
        "decoded payload begins neither '{' nor a zlib header — unrecognized encoding",
    };
  }
  let text;
  try {
  ...
  if (major === undefined || Number(major) !== 1)
    return {
      rejected: "a coffeejson major version this consumer does not support",
    };
  ...
  if (!carries("beans") && !carries("recipes"))
    return {
      rejected: "neither a non-empty beans nor a non-empty recipes collection",
    };
  return { document: doc };
};
...
const { DECODE_ERROR_KINDS, decodeScanned } = await import(
...
const { vectors } = JSON.parse(
  readFileSync(
    join(root, "fixtures", "transport", "scan-vectors.json"),
    "utf8",
  ),
);
for (const vector of vectors) {
  ...
  if (vector.expect === "document") {
    JSON.stringify(pkg.document) === JSON.stringify(vector.document)
      ...
      : fail(
          `scan ${vector.name} — @coffeejson/core agrees`,
          pkg.document
            ? "decoded a different document"
            : `rejected as "${pkg.rejectedKind}"`,
        );
  } else {
    ...
    DECODE_ERROR_KINDS.includes(vector.kind)
      ...
      : fail(
          `scan ${vector.name} names a vended error kind`,
          vector.kind === undefined
            ? "the vector states no kind"
            : `no such kind: "${vector.kind}"`,
        );
    pkg.rejectedKind === vector.kind
      ? ok(`scan ${vector.name} — @coffeejson/core rejects for the same reason`)
      : fail(
          `scan ${vector.name} — @coffeejson/core rejects for the same reason`,
  ...
  if (vector.expect === "document") {
    if (!outcome.document) {
      fail(
        `scan ${vector.name}`,
        `rejected (${outcome.rejected}), but this vector must decode`,
      );
      continue;
    }
    JSON.stringify(outcome.document) === JSON.stringify(vector.document)
      ...
      : fail(
          `scan ${vector.name}`,
          "decoded, but the document differs from the vector's expectation",
        );
    ...
    outcome.rejected === vector.reason
      ...
      : fail(
          `scan ${vector.name}`,
          `rejected as "${outcome.rejected}", but the vector states "${vector.reason}"`,
        );
  }
}
...
const bomBytes = readFileSync(
  join(root, "fixtures", "transport", "bom-prefixed-file.json"),
);
const mark = [0xef, 0xbb, 0xbf];
mark.every((b, i) => bomBytes[i] === b)
  ? ok("bom-prefixed-file.json still begins with a byte-order mark")
  : fail(
      "bom-prefixed-file.json still begins with a byte-order mark",
