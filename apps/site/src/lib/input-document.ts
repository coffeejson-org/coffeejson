 ...
 */
export function documentFromInput(text: string, emptyMessage: string): { doc?: unknown; error?: string } {
  const trimmed = text.trim();
  if (!trimmed) return { error: emptyMessage };
  if (/^https?:\/\//.test(trimmed)) {
    const scanned = decodeScanned(trimmed);
    return scanned.ok ? { doc: scanned.document } : { error: `Payload problem: ${stated(scanned.error.kind, scanned.error.detail)}` };
  }
  let parsed: unknown;
  try { parsed = JSON.parse(trimmed); }
  catch (e) { return { error: `Not valid JSON: ${(e as Error).message}` }; }
  ...
  return envelope.ok ? { doc: envelope.document } : { error: `Envelope problem: ${stated(envelope.error.kind, envelope.error.detail)}` };
}
