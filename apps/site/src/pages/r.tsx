...
import { decodePayload, payloadFromLocation, normalize, scopeToBean, scopeToRecipe } from "@coffeejson/core";
...
import { Fail, SaveCta, header } from "./r-shared";
import type { Mode } from "./r-shared";
import { Brew } from "./r-brew";

export function App({ doc, initialMode = { kind: "view" } }: { doc: DecodedDocument; initialMode?: Mode }) {
  ...
  const scoped = useMemo(() => {
    const aligned = (doc.recipes?.length ?? 0) === n.recipes.length
  ...
  useEffect(() => { injectJsonLd(docJsonLd(doc)); }, [doc]);
  ...
  return (
    <>
      ...
      <div className="cj-view">
        {n.beans.map((b, i) => (
          <div className="cj-recipe-block" key={`b${i}`}>
            <BeanCard bean={b} />
            {scoped?.beans[i] ? (
              <SaveCta doc={scoped.beans[i]!} prominent={false} variant="row"
        ...
        {n.recipes.map((r: NormalizedRecipe, i) => (
          <div className="cj-recipe-block" key={`r${i}`}>
            <RecipeCard recipe={r} />
            {r.steps.length > 0 ? (
              <p><button className="btn" onClick={() => setMode({ kind: "brew", index: i })}>Start brewing</button></p>
            ) : null}
            {scoped?.recipes[i] ? (
              <SaveCta doc={scoped.recipes[i]!} prominent={false} variant="row"
                label={scoped.recipes[i]!.beans?.length
      ...
      <p className="muted">Powered by <a href="/">CoffeeJSON</a>, an open format for coffee recipes.</p>
    </>
  );
}
...
function FromPayload({ raw }: { raw: string }) {
  ...
  if (!result.ok) return <Fail kind={result.error.kind} detail={result.error.detail} />;
  ...
  if (n.beans.length === 0 && n.recipes.length === 0) return <Fail kind="empty_document" />;
...
function FromSlug({ link }: { link: ShortLink }) {
  const [state, setState] = useState<{ raw: string } | { missing: true } | null>(null);
  useEffect(() => {
    let live = true;
    Promise.all([
      ...
      const payload = payloadForShortLink(
        cards.default as CorpusEntry[], documents.default as DocumentIndex, link);
...
const mount = typeof document === "undefined" ? null : document.querySelector<HTMLElement>("#app");
