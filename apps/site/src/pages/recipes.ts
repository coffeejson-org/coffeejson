...
function wire(): void {
  ...
  q.addEventListener("input", () => { filters = { ...filters, q: q.value }; renderPreservingFocus(); });
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-view]").forEach((t) =>
    t.addEventListener("click", () => {
      ...
      document.querySelector<HTMLButtonElement>(`[data-view="${view}"]`)?.focus();
    }));
  ...
  document.querySelectorAll<HTMLButtonElement>(".chip[data-kind]").forEach((c) =>
    c.addEventListener("click", () => {
      ...
      document.querySelector<HTMLButtonElement>(`.chip[data-kind="${kind}"][data-value="${value}"]`)?.focus();
    }));
  document.querySelectorAll<HTMLButtonElement>("[data-copy]").forEach((b) =>
    b.addEventListener("click", async () => {
      const s = shareable(b.dataset["copy"]!)!;
      await navigator.clipboard.writeText(`${location.origin}/r/?d=${s.payload}`);
      flash(b, "Copied");
    }));
  document.querySelectorAll<HTMLButtonElement>("[data-dl]").forEach((b) =>
    b.addEventListener("click", () => {
      ...
      const blob = new Blob([JSON.stringify(decoded.document, null, 2)], { type: MEDIA_TYPE });
      const a = Object.assign(document.createElement("a"), { href: URL.createObjectURL(blob), download: s.file });
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-qr]").forEach((b) =>
    b.addEventListener("click", async () => {
      ...
      if (slot.innerHTML) { closeQr(id); return; }
      ...
      slot.innerHTML = qrPanel(svg, {
        ...
        caption: `Scan to open on a phone${short
  ...
  document.addEventListener("keydown", (ev) => {
    ...
    const open = document.querySelector<HTMLElement>("[data-qr-slot]:not(:empty)");
...
function renderPreservingFocus(): void {
  const pos = (document.querySelector<HTMLInputElement>("#q"))!.selectionStart ?? 0;
  render();
  const q = document.querySelector<HTMLInputElement>("#q")!;
