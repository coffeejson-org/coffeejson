...
import authoringSchema from "@coffeejson/core/schema/authoring" with { type: "json" };
...
const die = (msg) => { console.error(`gen: ${msg}`); process.exit(1); };
...
const warnDense = (subject, urlLength) => {
  if (urlLength > 2500)
    console.warn(`gen: WARN ${subject} encoded URL is ${urlLength} bytes (> 2500) — QR density risk; trim the document, never truncate`);
};
...
export const INDEXABLE_PATHS = [
  "/", "/validator/", "/r/", "/recipes/", "/generate/", "/for-ai-agents/", "/implementations/",
  "/beans/", "/showcase/",
];
export const indexableUrls = () => INDEXABLE_PATHS.map((p) => `${SITE_URL}${p}`);
...
const SPEC_DOCS = [
  ["Overview", "docs/spec/01-overview.md", "What CoffeeJSON is, the design rules, and how to read the spec"],
  ["Document envelope", "docs/spec/02-envelope.md", "The top-level shape: the `coffeejson` version string, `recipes[]`, `beans[]`, `tastings[]`"],
  ["Recipe", "docs/spec/03-recipe.md", "Recipe fields: quantities, `basis`, steps, grind, gear, attribution, scaling"],
  ["Bean", "docs/spec/04-bean.md", "Bean identity: roaster, origin, process, roast level, altitude"],
  ["Tasting", "docs/spec/05-tasting.md", "How a brewed cup turned out: the drinker's impression, and what an instrument read"],
  ["Vocabularies & registries", "docs/spec/06-vocabularies.md", "The controlled value sets, how they extend, and the fallback rules"],
  ["Versioning & conformance", "docs/spec/07-versioning.md", "Compatibility contract, reserved extensions, what conformance requires"],
];

const GUIDE_DOCS = [
  ["Integration guide", "docs/integration-guide.md", "The consumer and producer checklists: intake, decode, fallback rules, preservation, authoring lint, share links"],
  ["Transport", "docs/transport.md", "Carrying a document in a URL or QR code: encoding, size limits, fallbacks"],
];

const link = (title, url, description) => `- [${title}](${url}): ${description}`;
...
export function rewriteDocLinks(markdown, docPath) {
  ...
  return markdown.replace(/\]\(([^)]+)\)/g, (whole, inside) => {
    ...
    let url;
    if (resolved === "docs/schema/coffeejson-1.0.schema.json") url = `${SITE_URL}/schema/1.0`;
    else if (resolved === "docs/schema/coffeejson-1.0.authoring.schema.json") url = `${SITE_URL}/schema/authoring/1.0`;
    else if (SERVED_MD_SET.has(resolved) || resolved.startsWith("registries/")) url = `${SITE_URL}/${resolved}`;
...
export function buildLlmsTxt() {
  return [
    ...
    link("Producer-role registry", `${SITE_URL}/registries/producer-roles.json`, "Recommended values for a party's role — an open set, so any other string is valid"),
    link("Implementations registry", `${SITE_URL}/registries/implementations.json`, "Self-declared implementations — apps, services, libraries — and the transport surfaces each reads and writes"),
    link("Full spec as one document", `${SITE_URL}/llms-full.txt`, "Every chapter concatenated, for loading into a single context"),
    "",
    "## Tools",
    "",
    link("Validator", `${SITE_URL}/validator/`, "Paste a document, URL, or file and check it against the schema — runs entirely in the browser"),
    link("Recipe directory", `${SITE_URL}/recipes/`, "Transcribed real-world recipes as valid documents; each names and links its source. The same corpus by bag at ?view=beans — bean identity, roaster, origin, and the recipes brewed with it"),
...
export function buildLlmsFullTxt(read = (p) => readFileSync(join(repo, p), "utf8")) {
  const parts = [
  ...
  for (const [, path] of [...SPEC_DOCS, ...GUIDE_DOCS]) {
    parts.push(`<!-- source: ${path} -->`, "", rewriteDocLinks(read(path).trimEnd(), path), "", "---", "");
  }
  return parts.join("\n");
}
...
function readCorpus() {
  ...
  const files = readdirSync(dir).filter((f) => f.endsWith(".json") && f !== "catalog.json");
  const fileSlugs = new Set(files.map((f) => basename(f, ".json")));
  const catSlugs = new Set(catalog.recipes.map((r) => r.slug));
  for (const s of catSlugs) if (!fileSlugs.has(s)) die(`catalog names ${s}.json which does not exist`);
  for (const s of fileSlugs) if (!catSlugs.has(s)) die(`${s}.json has no catalog entry`);

  return catalog.recipes.map((entry) => {
    const doc = JSON.parse(readFileSync(join(dir, `${entry.slug}.json`), "utf8"));
    if (!validate(doc)) die(`${entry.slug}.json is not a valid CoffeeJSON document:\n` +
...
export function buildIndex(corpus = readCorpus()) {
  ...
  return corpus.flatMap(({ entry, doc }) => {
    ...
    if (recipes.length === 0) {
      if (beans.length === 0) die(`${entry.slug}.json carries neither a recipe nor a bean`);
      beans.forEach((b, i) => {
        if (!b.roaster?.name) die(`${entry.slug}.json bean ${i} carries no roaster name — a bean-only document must credit who roasted it`);
        if (!b.url) die(`${entry.slug}.json bean ${i} carries no url — a bean-only document must link its source`);
      });
      warnDense(entry.slug, `${SITE_URL}${SHARE_PATH}`.length + encodePayload(doc).length);
      return [];
    }
    ...
    if (projected.length !== recipes.length)
      die(`${entry.slug}.json: ${recipes.length} recipes on the wire, ${projected.length} projected`);
    ...
    if (!r.author?.name) die(`${entry.slug}.json recipe ${i} carries no author — corpus recipes are transcriptions and must credit one`);
    if (!r.based_on) die(`${entry.slug}.json recipe ${i} carries no based_on — corpus recipes must link their source`);
    return {
...
function beanSlugHalf(name, roasterSlug, docSlugs) {
  ...
  if (docSlugs.length !== 1)
    die(`bean "${name}": a non-Latin name needs exactly one document to take its ` +
        `slug from, but has ${docSlugs.length} (${docSlugs.join(", ") || "none"})`);
  const prefix = `${roasterSlug}-`;
  if (!docSlugs[0].startsWith(prefix))
    die(`bean "${name}": document slug "${docSlugs[0]}" does not start with "${prefix}", ` +
...
const originOf = (nb) => (nb?.originItems ?? [])
  .map((it) => originLine(it, defaultLabels)).filter(Boolean).join(" + ");

const roastOf = (nb, withDate) => [
  label(defaultLabels.roastLevels, nb?.roastLevel),
  nb?.roastAgtron !== null && nb?.roastAgtron !== undefined ? `Agtron ${nb.roastAgtron}` : "",
...
export function buildBeansIndex(corpus = readCorpus()) {
  ...
  const byKey = new Map();
  const slot = (key) => {
    if (!byKey.has(key)) byKey.set(key, { key, instances: [], recipes: [], documents: [] });
  ...
  const keyOf = (b) => (b?.roaster?.name && b?.name ? `${slugify(b.roaster.name)}/${slugify(b.name)}` : null);

  corpus.forEach(({ entry, doc }, order) => {
    const beans = (doc.beans ?? []).filter((b) => keyOf(b));
    for (const bean of beans) {
      slot(keyOf(bean)).instances.push({ bean, order, coffeejson: doc.coffeejson,
        transcribed: entry.attribution?.transcribed ?? "" });
    ...
    for (const r of doc.recipes ?? []) {
      ...
      const targets = r.bean_ref !== undefined
        ? beans.filter((b) => b.id === r.bean_ref)
        : beans;
      const member = { slug: entry.slug, title: r.title, methodLabel: method(r.method) };
  ...
  return [...byKey.values()].map(({ key, instances, recipes, documents }) => {
    const [{ bean: b, coffeejson }] = [...instances].sort((x, y) =>
    ...
    warnDense(`bean ${key}`, `${SITE_URL}${SHARE_PATH}`.length + payload.length);
    ...
    return {
      ...
      slug: beanPageSlug(b.roaster.name, b.name, documents.map((d) => d.slug)),
 ...
 */
export function buildDocuments(corpus = readCorpus(), index = buildIndex(corpus)) {
  const cardPayloads = new Map();
  for (const e of index) if (!cardPayloads.has(e.slug)) cardPayloads.set(e.slug, e.payload);
  const out = {};
...
const pageHead = ({ title, description, url, ld }) => `<head>
    ...
    <link rel="stylesheet" href="/src/styles.css" />${ld?.length ? `
    <script type="application/ld+json">${
      JSON.stringify(ld.length === 1 ? ld[0] : ld).replace(/</g, "\\u003c")}</script>` : ""}
  </head>`;

export const corpusPagePath = (slug) => `/recipes/${slug}/`;
/** Document slugs that produce at least one recipe card, in catalog order. */
export const corpusPageSlugs = (index = buildIndex()) => [...new Set(index.map((e) => e.slug))];
...
export const allIndexableUrls = (index = buildIndex(), beans = buildBeansIndex()) =>
  [...indexableUrls(), ...corpusPageUrls(index), ...beanPageUrls(beans)];

const factsOf = (n) => {
  const out = fmt(n.coffee) + (n.isEspresso
    ? (n.yield ? ` → ${fmt(n.yield)}` : "")
    : (n.water ? ` → ${fmt(n.water)}` : ""));
  ...
  return [out, formatRatio(n.ratio), fmt(n.waterTemp),
    n.finishS !== null ? `${fmtClock(n.finishS)} total` : "",
    grindSize ? `${grindSize} grind` : ""].filter(Boolean).join(" · ");
};

const stepsHtml = (n) => n.steps.length === 0 ? "" :
  `<ol class="steps">${n.steps.map((s) => {
    const when = s.atS !== null ? fmtClock(s.atS) : "";
    const to = fmt(s.toWater);
    const body = s.text || label(defaultLabels.stepKinds, s.kind);
    return `<li><span class="t">${esc(when)}</span><span>${esc(body)}</span>` +
      (to ? `<span class="to muted">${esc(to)}</span>` : "") + `</li>`;
  }).join("")}</ol>`;
...
const beanHtml = (b) => {
  ...
  const rest = [processLine(nb?.process ?? [], defaultLabels), roastOf(nb, false)].filter(Boolean);
  return `<section class="card bean-card"><h2>The bag</h2>
    ...
    ${(nb?.roasterNotes ?? []).length
      ? `<p class="muted"><em>${esc(nb.roasterNotes.join(" · "))}</em></p>` : ""}</section>`;
};
...
export function corpusPageMeta(entry, doc) {
  ...
  const subject = bean ? `${bean.roaster?.name ? `${bean.roaster.name} ` : ""}${bean.name}` : label;
...
export function buildCorpusPage(entry, doc) {
  ...
  const payload = encodePayload(doc);
  const ld = recipes.map((_, i) => recipeJsonLd(doc, i, { url: multi ? `${url}#recipe-${i + 1}` : url }))
    .filter(Boolean);

  const sections = recipes.map((_, i) => {
    const n = projected[i];
    return `
    <section${multi ? ` id="recipe-${i + 1}"` : ""}>
      ...
      <p class="muted">${esc(n.author?.name ?? "")} · ${esc(method(n.method))}${
        ...
        n.brewerLabel && n.brewerLabel.toLowerCase() !== methodLabel(n.method).toLowerCase()
          ? ` · ${esc(n.brewerLabel)}` : ""}</p>
      <p>${esc(factsOf(n))}</p>
      ...
      ${multi ? `<div data-share-slot data-payload="${esc(encodePayload(scopeToRecipe(doc, i)))}"
...
export function buildBeanPage(bean) {
  ...
  const facts = [
    ...
    ["Roast", bean.roast], ["Roaster's notes", bean.notes],
  ].filter(([, v]) => v)
    .map(([k, v]) => `<p><span class="muted">${k}</span> — ${esc(v)}</p>`).join("");
  ...
  const byDoc = new Map();
  for (const r of bean.recipes) byDoc.set(r.slug, [...(byDoc.get(r.slug) ?? []), r]);
  const brews = byDoc.size
    ? `<h2>Brewed with it</h2><ul class="bean-recipes">${[...byDoc].map(([slug, rs]) => rs.map((r) =>
        `<li><a href="${esc(corpusPagePath(slug))}">${esc(r.title)}</a>
          <span class="muted">${esc(r.methodLabel)}</span></li>`).join("")).join("")}</ul>`
  ...
  const sources = `<h2>Sources</h2><ul class="bean-sources">${bean.documents.map((d) => `
    <li>${d.url ? `<a href="${esc(d.url)}" rel="noopener">${esc(d.source_label)}</a>`
          : esc(d.source_label)}
      <span class="muted">transcribed ${esc(d.transcribed)}</span>
      ${byDoc.has(d.slug) ? ` · <a href="${esc(corpusPagePath(d.slug))}">recipes</a>` : ""}</li>`).join("")}</ul>`;
...
export const beanPageSlugs = (beans = buildBeansIndex()) => beans.map((b) => b.slug);
...
export function buildBeanPages(beans = buildBeansIndex()) {
  ...
  for (const b of beans) {
    if (!b.slug)
      die(`roaster "${b.roaster?.name}" has no slug (bag "${b.name}"). Add it to ` +
          `ROASTER_SLUG in gen.mjs — the name is not derivable from itself, ` +
          `which is the whole reason the table exists.`);
    if (seen.has(b.slug))
      die(`bean slug collision: "${b.slug}" is claimed by both ${seen.get(b.slug)} and ${b.key}`);
    seen.set(b.slug, b.key);
  }
  return beans.map((b) => ({ slug: b.slug, path: beanPagePath(b.slug), html: buildBeanPage(b) }));
}

/** Every corpus page, keyed by the directory it is written to. */
export function buildCorpusPages(corpus = readCorpus(), index = buildIndex(corpus)) {
...
const isMain = process.argv[1] && fileURLToPath(new URL(process.argv[1], "file://")) === fileURLToPath(import.meta.url);
if (isMain) { await (async () => {
  const index = buildIndex();
  mkdirSync(join(site, "src/generated"), { recursive: true });
  writeFileSync(join(site, "src/generated/recipes-index.json"), JSON.stringify(index, null, 2));
  console.log(`gen: recipes-index.json — ${index.length} recipes`);

  const documents = buildDocuments();
  writeFileSync(join(site, "src/generated/documents-index.json"), JSON.stringify(documents, null, 2));
  console.log(`gen: documents-index.json — ${Object.keys(documents).length} publications a card does not already carry whole`);

  const beans = buildBeansIndex();
  writeFileSync(join(site, "src/generated/beans-index.json"), JSON.stringify(beans, null, 2));
  console.log(`gen: beans-index.json — ${beans.length} beans from ` +
  ...
  writeFileSync(join(site, "src/generated/corpus-counts.json"), JSON.stringify(counts, null, 2) + "\n");
  console.log(`gen: corpus-counts.json — ${counts.recipes} recipes · ` +
  ...
  writeFileSync(join(site, "public/schema/coffeejson-1.0.schema.json"), schemaBytes);
  const authoringBytes = readFileSync(join(repo, "docs/schema/coffeejson-1.0.authoring.schema.json"));
  writeFileSync(join(site, "public/schema/authoring/1.0"), authoringBytes);
  writeFileSync(join(site, "public/schema/coffeejson-1.0.authoring.schema.json"), authoringBytes);
  console.log("gen: schema → public/schema/1.0 + /schema/authoring/1.0 (+ aliases)");
  ...
  for (const { $id: id } of [runtimeSchema, authoringSchema]) {
    if (!id?.startsWith(`${SITE_URL}/`)) die(`$id ${id} is not on ${SITE_URL}`);
    const served = join(site, "public", id.slice(SITE_URL.length));
    if (!existsSync(served)) die(`$id ${id} does not resolve — nothing served at ${served}`);
  }
  ...
  const sweepGenerated = (dir) => {
    ...
    for (const name of gone) rmSync(join(site, dir, name), { recursive: true, force: true });
    return gone;
  };
  ...
  console.log(`gen: corpus pages — ${pages.length} documents → /recipes/<slug>/`);
  ...
  console.log(`gen: bean pages — ${beanPages.length} identities → /beans/<slug>/`);

  const all = allIndexableUrls(index, beans);
  writeFileSync(join(site, "public/sitemap.xml"), buildSitemap(all));
  console.log(`gen: sitemap.xml — ${all.length} URLs ` +
  ...
  for (const f of ["gear.json", "varietals.json", "addition-types.json", "producer-roles.json", "implementations.json"])
    writeFileSync(join(site, "public/registries", f), readFileSync(join(repo, "registries", f)));
  console.log("gen: registries → public/registries/ (gear, varietals, addition types, producer roles, implementations)");
  ...
  for (const path of SERVED_MD) {
    ...
    writeFileSync(dest, rewriteDocLinks(readFileSync(join(repo, path), "utf8"), path));
  }
  console.log(`gen: spec markdown → public/docs/ (${SERVED_MD.length} files, links rewritten for the site root)`);
  ...
  console.log(`gen: llms.txt + llms-full.txt (${Math.round(full.length / 1024)} KB)`);
  ...
  if (demoUrl.length > 2500) die(`demo QR URL is ${demoUrl.length} bytes — trim the document`);
  mkdirSync(join(site, "public/demo"), { recursive: true });
  writeFileSync(join(site, "public/demo/tetsu-kasuya-4-6.svg"),
    await QRCode.toString(demoUrl, { type: "svg", errorCorrectionLevel: "M", margin: 2 }));
  await QRCode.toFile(join(site, "public/demo/tetsu-kasuya-4-6.png"), demoUrl,
