...
const validateStrict = ajv.compile(load("docs/schema/coffeejson-1.0.authoring.schema.json"));
const validateRuntime = ajv.compile(load("docs/schema/coffeejson-1.0.schema.json"));
...
describe("/for-ai-agents examples", () => {
  it.each(EXAMPLES.map((e, i) => [i, e.prompt] as const))(
    ...
    (i) => {
      ...
      const errs = (validateStrict.errors ?? []).map((e) => `${e.instancePath || "/"} ${e.message}`);
  ...
  it("covers the three shapes an agent is actually asked for", () => {
    ...
    expect(docs.some((d) => !("steps" in ((d["recipes"] as never[])?.[0] ?? {})))).toBe(true);
    expect(docs.some((d) => "steps" in ((d["recipes"] as never[])?.[0] ?? {}))).toBe(true);
  ...
  it("the system prompt names the authoring schema and the cumulative-water trap", () => {
    ...
    expect(SYSTEM_PROMPT).toContain("https://coffeejson.org/schema/authoring/1.0");
  ...
  it("every pitfall's WRONG form really is rejected by the authoring schema", () => {
    ...
    expect(validateStrict({ coffeejson: "1.0", recipes: [{ title: "x", coffee: 15 }] })).toBe(false);
    expect(
      validateStrict({ coffeejson: "1.0", recipes: [{ title: "x", coffee: { value: 15, unit: "g" } }] }),
    ).toBe(false);
