...
describe("the authoring lint over documents the runtime schema accepts", () => {
  for (const [name, doc] of docs("fixtures/valid")) {
    ...
    if (bare === newerMinor)
      test(`${name} is a later minor, so the lint says nothing`, () => expect(lintDocument(doc)).toEqual([]));
    else if (lenient.has(bare))
      test(`${name} draws an authoring note`, () => expect(lintDocument(doc).length).toBeGreaterThan(0));
    else test(`${name} passes the lint too`, () => expect(lintDocument(doc)).toEqual([]));
  }
  for (const [name, doc] of docs("recipes").filter(([n]) => !n.endsWith("catalog.json")))
    test(`${name} passes the lint too`, () => expect(lintDocument(doc)).toEqual([]));
  ...
  test("the reserved vendor member draws no note, at any depth", () => {
    const vendor = {
      coffeejson: "1.0",
      recipes: [{
        title: "x", coffee: { value: 15, unit: "gram" }, water: { value: 250, unit: "gram" },
        brewer: { id: "custom", label: "Bench dripper", ext: { "app.example": { asset_tag: "D-14" } } },
...
describe("what a note actually says", () => {
  const recipe = (extra: Record<string, unknown>) => ({
    coffeejson: "1.0",
    recipes: [{ title: "x", coffee: { value: 15, unit: "gram" }, water: { value: 250, unit: "gram" }, ...extra }],
  });
  ...
  test("each note names the member it is about", () => {
    const notes = lintDocument(recipe({ watter_temp: { value: 93, unit: "celsius" }, grindd: "fine" }));
  ...
  test("a missing required field is not named twice", () => {
    const missing = validateDocument({ coffeejson: "1.0", recipes: [{ title: "x" }] });
    const required = missing.filter((i) => i.message.startsWith("must have required property"));
