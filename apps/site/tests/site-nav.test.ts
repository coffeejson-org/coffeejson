...
const NAV_ORDER = ["Browse", "Showcase", "Implementations", "Validator", "Generate", "GitHub"];
...
const labelsIn = (html: string): string[] =>
  [...html.matchAll(/>(Browse|Showcase|Implementations|Validator|Generate|GitHub)</g)].map((m) => m[1]!);
...
test("a page marks itself as current exactly when it has a nav entry", () => {
  ...
  for (const current of ["/", "/beans/", "/for-ai-agents/", "/recipes/some-slug/"])
    expect((siteHeader(current).match(/aria-current="page"/g) ?? []).length, current).toBe(0);
});

test("the wordmark links home everywhere but home", () => {
  expect(siteHeader("/")).toContain("<header class=\"site-header\"><strong>CoffeeJSON</strong>");
  expect(siteHeader("/validator/")).toContain('<a href="/"><strong>CoffeeJSON</strong></a>');
});
...
test("no page module hand-writes the masthead", () => {
  const handWritten = readdirSync(pagesDir)
    ...
    .filter((f) => /<header[^>]*site-header/.test(readFileSync(join(pagesDir, f), "utf8")));
...
test("the generator renders the masthead through the shared function", () => {
  const gen = readFileSync(fileURLToPath(new URL("../tools/gen.mjs", import.meta.url)), "utf8");
