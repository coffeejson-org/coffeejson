...
import {
  buildRecipe, buildDocument, perPourAmounts, stepsNonDecreasing,
  emptyRecipeForm, emptyStepForm,
  buildBean, emptyBeanForm, emptyOriginItemForm,
  documentToState, collectDroppedPaths,
} from "../src/lib/builder";
import { validateDocument } from "../src/lib/validate";
import type { RecipeFormState, BeanFormState, BuilderState } from "../src/lib/builder";

const filterForm = (): RecipeFormState => ({
  ...emptyRecipeForm(),
  title: "Everyday V60", method: "pour_over", brewerLabel: "Hario V60",
  coffee: "15", water: "250", ratio: "", waterTempC: "94", grindSetting: "medium",
  steps: [
    { ...emptyStepForm(), at_s: "0", cumulative: "50", instruction: "bloom" },
    { ...emptyStepForm(), at_s: "45", cumulative: "150", instruction: "pour" },
    { ...emptyStepForm(), at_s: "90", cumulative: "250", instruction: "finish" },
  ],
  finish_s: "180",
});
...
const espressoForm = (): RecipeFormState => ({
  ...emptyRecipeForm(),
  title: "House Espresso", method: "espresso", coffee: "18",
  yield: "36", pressure: "9", preinfusion_s: "5", basketLabel: "18g VST",
  finish_s: "28",
});
...
describe("buildRecipe is method-aware", () => {
  ...
  test("empty optional fields are omitted (no undefined/empty keys emitted)", () => {
    const r = buildRecipe({ ...emptyRecipeForm(), title: "Bare", method: "", coffee: "20", water: "320" });
    expect(r).toEqual({ title: "Bare", coffee: { value: 20, unit: "gram" }, water: { value: 320, unit: "gram" } });
  });
});
...
describe("valid by construction (property): complete forms validate", () => {
  const forms: RecipeFormState[] = [filterForm(), espressoForm(), {
    ...emptyRecipeForm(), title: "Immersion", method: "french_press", coffee: "30", water: "500",
    steps: [{ ...emptyStepForm(), kind: "press", instruction: "press slowly" }],
  }];
  for (const f of forms)
    test(`buildDocument([${f.title}]) validates against the schema`, () =>
      expect(validateDocument(buildDocument({ beanForms: [], recipeForms: [f] }))).toEqual([]));
});

describe("buildBean", () => {
  test("emits identity + origin items with per-component name/process", () => {
    const f: BeanFormState = { ...emptyBeanForm(), name: "Monarch", roaster: "Onyx Coffee Lab",
      url: "https://onyxcoffeelab.com/products/monarch", process: "washed", roastLevel: "dark",
      description: "blend", origin: [
        { ...emptyOriginItemForm(), name: "The Queen", country: "CO" },
        { ...emptyOriginItemForm(), name: "Alaka", country: "ET", process: "natural" }] };
    const b = buildBean(f);
    expect(b.name).toBe("Monarch");
    expect(b.origin?.items?.[1]).toMatchObject({ name: "Alaka", country: "ET", process: ["natural"] });
  ...
  test("a single-item origin array emits origin.type \"single\" (not \"single_origin\")", () => {
    const f: BeanFormState = { ...emptyBeanForm(), name: "Solo Estate",
      origin: [{ ...emptyOriginItemForm(), name: "La Esperanza", country: "CO" }] };
...
describe("buildDocument(state) — bundle", () => {
  test("emits beans only when present; recommended flags survive", () => {
    const state: BuilderState = { beanForms: [], recipeForms: [
      { ...emptyRecipeForm(), title: "A", method: "pour_over", coffee: "20", water: "320", recommended: true }] };
...
describe("documentToState (edit mode) round-trips through buildDocument", () => {
  const sunriseBean = (): BeanFormState => ({ ...emptyBeanForm(), name: "Sunrise Reserve", roaster: "Northbound Roasters",
    url: "https://example.com/sunrise-reserve", process: "honey", roastLevel: "medium_dark",
    description: "A bright, syrupy blend.",
    origin: [
      { ...emptyOriginItemForm(), name: "La Esperanza", country: "CO", region: "Huila", process: "washed", percentage: "60" },
      { ...emptyOriginItemForm(), name: "Guji Hambela", country: "ET", process: "natural", percentage: "40" }] });

  const sunriseFilter = (): RecipeFormState => ({ ...emptyRecipeForm(), title: "Sunrise V60",
    method: "pour_over", brewerLabel: "Hario V60", coffee: "15", water: "250", ratio: "16.7",
    waterTempC: "94", grindSetting: "medium-fine", recommended: true,
    basedOn: "https://example.com/sunrise-v60-guide",
    steps: [
      { ...emptyStepForm(), at_s: "0", cumulative: "50", instruction: "bloom" },
      { ...emptyStepForm(), at_s: "45", cumulative: "150", instruction: "first pour" },
      { ...emptyStepForm(), at_s: "90", cumulative: "250", instruction: "final pour" }],
    finish_s: "180" });

  const sunriseEspresso = (): RecipeFormState => ({ ...emptyRecipeForm(), title: "Sunrise Espresso",
    method: "espresso", coffee: "18", yield: "36", pressure: "9", preinfusion_s: "5",
    basketLabel: "18g VST", waterTempC: "93", grindSetting: "fine", finish_s: "28", recommended: true });

  test("a built bundle survives document → state → document unchanged & valid", () => {
    const original = buildDocument({ beanForms: [sunriseBean()], recipeForms: [sunriseFilter(), sunriseEspresso()] });
  ...
  test("populates espresso fields from an espresso document", () => {
    const doc = buildDocument({ beanForms: [], recipeForms: [{ ...emptyRecipeForm(), title: "Solo Shot",
      method: "espresso", coffee: "18", yield: "47", pressure: "9", finish_s: "27" }] });
...
describe("collectDroppedPaths (the drop-warning engine)", () => {
  ...
  test("names the recipe fields the form can't carry", () => {
    const dropped = roundTrip({
      coffeejson: "1.0",
      recipes: [{
        title: "t", coffee: { value: 15, unit: "gram" }, water: { value: 250, unit: "gram" },
        author: { name: "Someone" }, based_on: "https://example.com/post",
        notes: "long prose", lang: "en",
        grind: { setting: "22 clicks", size: "medium_fine", microns_approx: 700 },
      }],
    });
  ...
  test("an imported document's `generator` is named as dropped", () => {
    ...
    const dropped = roundTrip({
      ...
      recipes: [{ title: "t", coffee: { value: 15, unit: "gram" }, water: { value: 250, unit: "gram" } }],
    });
  ...
  test("a document the form fully carries reports nothing", () => {
    const dropped = roundTrip({
      coffeejson: "1.0",
      recipes: [{
        title: "t", method: "pour_over", coffee: { value: 15, unit: "gram" },
        water: { value: 250, unit: "gram" }, ratio: 16.7,
        water_temp: { value: 94, unit: "celsius" }, finish_s: 180,
        steps: [{ at_s: 0, to_water: { value: 50, unit: "gram" }, instruction: "bloom" }],
      }],
    });
...
describe("the brew filter is authorable", () => {
  test("a stated material emits a filter; a label rides along", () => {
    const r = buildRecipe({ ...emptyRecipeForm(), title: "V60", method: "pour_over", coffee: "15", water: "250",
      filterMaterial: "paper", filterLabel: "Hario tabbed" });
  ...
  test("a material with no label emits material alone, never an empty label", () => {
    const r = buildRecipe({ ...emptyRecipeForm(), title: "Press", method: "french_press", coffee: "30", water: "500",
  ...
  test("a filter survives document → state → document", () => {
    const doc = buildDocument({ beanForms: [], recipeForms: [{ ...emptyRecipeForm(), title: "V60",
      method: "pour_over", coffee: "15", water: "250", filterMaterial: "cloth", filterLabel: "Nel" }] });
