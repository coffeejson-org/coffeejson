...
import { beanPageSlug, beanPagePath, buildBeanPages, corpusPageSlugs, INDEXABLE_PATHS } from "../tools/gen.mjs";
...
test("every bean identity produces a distinct slug", () => {
  ...
  expect(new Set(slugs).size, `duplicates: ${
    slugs.filter((s, i) => slugs.indexOf(s) !== i).join(", ")}`).toBe(slugs.length);
});
...
test("every slug is ASCII, lowercase, kebab — a path, not a display name", () => {
  ...
  for (const b of beans) expect(b.slug, b.key).toMatch(/^[a-z0-9]+(-[a-z0-9]+)*$/);
});
...
test("a name written in another script takes the slug its document already used", () => {
  ...
  expect(beanPageSlug("サザコーヒー", "ゴールデンモカ", ["saza-golden-mocha"])).toBe("saza-golden-mocha");
  expect(beanPageSlug("ONIBUS COFFEE", "ケニア / ガトンボヤ", ["onibus-kenya-gatomboya"]))
...
test("a Latin name folds, and does not consult the document", () => {
  ...
  expect(beanPageSlug("Equator Coffees", "Golden Hour Blend", [])).toBe("equator-golden-hour-blend");
});
...
test("the three roaster names no fold can compute are table entries", () => {
  ...
  expect(beanPageSlug("FUGLEN COFFEE ROASTERS TOKYO", "X", [])).toBe("fuglen-x");
  expect(beanPageSlug("LIGHT UP COFFEE", "X", [])).toBe("lightup-x");
  expect(beanPageSlug("DRINK COFFEE DO STUFF", "X", [])).toBe("drink-coffee-do-stuff-x");
});
...
test("an unknown roaster yields no slug, and fails the BUILD rather than the projection", () => {
  ...
  expect(() => buildBeanPages([{ ...beans[0]!, slug: null } as never])).toThrow();
  // And every roaster the corpus actually has IS in the table.
  for (const b of beans) expect(b.slug, `${b.roaster.name} is missing from ROASTER_SLUG`).toBeTruthy();
});
...
test("attribution is the union of the documents describing the bag", () => {
  ...
  expect(monarch.documents.map((d) => d.slug).sort())
    .toEqual(["onyx-monarch", "onyx-monarch-fellow-series-1"]);
...
test("the hub is indexable and the per-bean pages ride the sitemap", () => {
  ...
  expect(INDEXABLE_PATHS.some((p: string) => p.includes("roaster="))).toBe(false);
