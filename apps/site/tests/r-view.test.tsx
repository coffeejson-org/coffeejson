...
const bag = JSON.parse(readFileSync(new URL("../../../fixtures/valid/bag-to-brew.json", import.meta.url), "utf8"));
const pourOver = JSON.parse(readFileSync(new URL("../../../fixtures/valid/typical-pour-over.json", import.meta.url), "utf8"));

test("view mode: bean card, then all recipe cards stacked; Recommended badge shown", () => {
  const html = renderToStaticMarkup(<App doc={bag} initialMode={{ kind: "view" }} />);
  expect(html).toContain('class="cj-view"');
  expect(html.indexOf("cj-bean-card")).toBeGreaterThan(-1);
  expect(html.indexOf("cj-bean-card")).toBeLessThan(html.indexOf("cj-recipe-card"));
...
test("single-recipe doc with steps renders one card + Start brewing", () => {
  const html = renderToStaticMarkup(
    <App
      doc={{ coffeejson: "1.0", recipes: [{ title: "Solo", coffee: { value: 15, unit: "gram" }, steps: [{ at_s: 0, instruction: "Bloom" }] }] }}
...
test("multi-pour recipe renders the columnar step schedule", () => {
  const html = renderToStaticMarkup(<App doc={pourOver} initialMode={{ kind: "view" }} />);
...
test("step-less recipe: no Start brewing button", () => {
  const html = renderToStaticMarkup(
    <App
      doc={{ coffeejson: "1.0", recipes: [{ title: "No steps", coffee: { value: 15, unit: "gram" } }] }}
      initialMode={{ kind: "view" }}
    />,
  );
...
test("a publication offers a share row per card, and a lone recipe does not", () => {
  const multi = renderToStaticMarkup(<App doc={bag} initialMode={{ kind: "view" }} />);
  // bag-to-brew: one bean + its recipes, so one row each.
  expect((multi.match(/scoped-share/g) ?? []).length).toBe(1 + bag.recipes.length);
  ...
  const solo = renderToStaticMarkup(
    <App
      doc={{ coffeejson: "1.0", recipes: [{ title: "Solo", coffee: { value: 15, unit: "gram" } }] }}
      initialMode={{ kind: "view" }}
    />,
  );
  ...
  const html = renderToStaticMarkup(
    <App
      doc={{
        ...
        recipes: [{ title: "Dwell", coffee: { value: 20, unit: "gram" },
...
test("the whole-document panel names a multi-recipe document a publication, not a recipe", () => {
  ...
  const solo = renderToStaticMarkup(
    <App doc={{ coffeejson: "1.0", recipes: [{ title: "Solo", coffee: { value: 15, unit: "gram" } }] }}
      initialMode={{ kind: "view" }} />,
  );
