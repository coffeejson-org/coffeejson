...
test("every card's document has a page, and every page has at least one card", () => {
  ...
  for (const slug of carded)
    expect(bySlug.has(slug), `${slug} has no page`).toBe(true);
  for (const p of pages)
    expect(carded.has(p.slug), `${p.slug} has no card`).toBe(true);
});

describe("head furniture", () => {
  it("every page declares its own canonical", () => {
    for (const p of pages) {
      expect(p.html, p.slug).toContain(
        `<link rel="canonical" href="${SITE_URL}${corpusPagePath(p.slug)}" />`,
      );
    }
  });
  ...
  it("gives every page a distinct title and description", () => {
    ...
    const descs = pages.map((p) =>
      grab(p.html, /<meta name="description" content="([^"]+)"/),
    );
...
describe("structured data", () => {
  it("every page carries Recipe JSON-LD naming the page's own URL", () => {
    for (const p of pages) {
      const ld = (/<script type="application\/ld\+json">(.*?)<\/script>/s.exec(
        p.html,
      ) ?? [])[1]!;
      ...
      for (const r of recipes) {
        ...
        expect(String(r.url).startsWith(page), `${p.slug}: ${r.url}`).toBe(
          true,
        );
...
describe("multi-recipe documents", () => {
  const multi = pages.filter(
    (p) => index.filter((e) => e.slug === p.slug).length > 1,
  );
  ...
  it("anchor each recipe and title themselves for the subject", () => {
    for (const p of multi) {
      ...
      for (let i = 1; i <= n; i++)
        expect(p.html, `${p.slug} #${i}`).toContain(`id="recipe-${i}"`);
      // The title names the subject, not whichever recipe was authored first.
      expect(p.html, p.slug).toMatch(
        /<title>.* — \d+ brew methods — CoffeeJSON<\/title>/,
      );
    }
  });
});
...
test("the page carries its documents inline, so it needs no index fetch", () => {
  for (const p of pages) {
    const payloads = [...p.html.matchAll(/data-payload="([^"]+)"/g)].map(
      (m) => m[1]!,
    );
    const cards = index.filter((e) => e.slug === p.slug);
    // One slot for the publication; on a multi-recipe page, one more per brew.
    expect(payloads.length, p.slug).toBe(
      cards.length > 1 ? cards.length + 1 : 1,
    );
...
test("the page-level share hands over the WHOLE publication, not its first recipe", () => {
  ...
  for (const p of pages) {
    ...
    const pageSlot =
      /data-share-slot data-payload="([^"]+)"[^>]*data-slug="[^"]*"(?![^>]*data-i=)/.exec(
        p.html,
      );
    expect(pageSlot, p.slug).toBeTruthy();
    const doc = JSON.parse(
      Buffer.from(pageSlot![1]!, "base64url").toString("utf8"),
    );
...
test("each brew section on a multi-recipe page shares only that brew", () => {
  for (const p of pages) {
    ...
    if (cards.length < 2) continue;
    const scoped = [
      ...p.html.matchAll(/data-payload="([^"]+)"[^>]*data-i="(\d+)"/g),
    ];
    ...
    for (const [, payload, i] of scoped) {
      ...
      expect(doc.recipes[0].title, `${p.slug} #${i}`).toBe(
        cards[Number(i) - 1]!.title,
      );
    }
  }
});
...
describe("corpusPageMeta", () => {
  it("titles a single-recipe document with its recipe", () => {
    const { title } = corpusPageMeta(
      {
        slug: "x",
        attribution: { source_label: "A source", transcribed: "2026-01-01" },
      },
      {
        coffeejson: "1.0",
        recipes: [
          { title: "One Cup", author: { name: "A" }, method: "pour_over" },
        ],
      },
    );
    ...
    const { title } = corpusPageMeta(
      {
        slug: "x",
        attribution: {
          source_label: "Roaster page",
          transcribed: "2026-01-01",
        },
      },
      {
        coffeejson: "1.0",
  ...
  it("falls back to the source label when a multi-recipe document has no bean", () => {
    const { title } = corpusPageMeta(
      {
        slug: "x",
        attribution: {
          source_label: "Linea Caffe — Brew Guides",
          transcribed: "2026-01-01",
        },
      },
      {
    ...
    expect(title).toBe(
      "Linea Caffe — Brew Guides — 2 brew methods — CoffeeJSON",
    );
  });
});
...
test("buildCorpusPage renders the grind size the projection carries", () => {
  const page = (size: unknown) =>
    buildCorpusPage(
      {
        slug: "x",
        attribution: { source_label: "S", transcribed: "2026-01-01" },
      },
      {
        coffeejson: "1.0",
        recipes: [
