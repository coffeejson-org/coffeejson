...
test("the generator derives no quantity the projection already derives", () => {
  ...
  for (const wire of ["r.ratio", "r.coffee", "r.water", "r.yield", "r.water_temp", "r.finish_s",
                      "r.grind"])
    expect(gen, wire).not.toContain(wire);
  expect(gen, "espresso is a basis question, not a method comparison")
...
test("the generator reads no shape the schema does not define", () => {
  ...
  expect(gen).not.toMatch(/Array\.isArray\(\w+\)\s*\?\s*\w+\s*:\s*\w+\s*\?\s*\[\w+\]/);
});
