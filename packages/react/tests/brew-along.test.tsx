...
test("composes clock + timeline + step card + countdown while brewing", () => {
  const html = renderToStaticMarkup(
    <BrewAlong
      ...
      brew={brewOf({
        elapsedS: 30,
        state: st({ currentIndex: 0, nextTimedIndex: 1 }),
      })}
    />,
  );
...
test("shows Resume when paused", () => {
  const html = renderToStaticMarkup(
    <BrewAlong
      recipe={v60}
      brew={brewOf({ running: false, state: st({ currentIndex: 0 }) })}
    />,
  );
...
test("surfaces the Done—next control only while awaiting a tap", () => {
  const awaiting = renderToStaticMarkup(
    <BrewAlong
      recipe={v60}
      brew={brewOf({ state: st({ currentIndex: 1, awaitingTap: true }) })}
    />,
  );
  expect(awaiting).toContain("Done, next step");
  const notAwaiting = renderToStaticMarkup(
    <BrewAlong
      recipe={v60}
      brew={brewOf({ state: st({ currentIndex: 1, awaitingTap: false }) })}
    />,
  );
