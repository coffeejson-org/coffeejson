...
test("full variant shows step n/total, time, signed pour delta, and total", () => {
  const html = renderToStaticMarkup(<StepCard recipe={v60} state={st({ currentIndex: 1 })} />);
...
test("the first targeted step shows its fill unsigned", () => {
  const html = renderToStaticMarkup(<StepCard recipe={v60} state={st({ currentIndex: 0 })} />);
...
test("compact variant drops the step/time head but keeps pour + total", () => {
  const html = renderToStaticMarkup(<StepCard recipe={v60} state={st({ currentIndex: 1 })} variant="compact" />);
...
test("finished state renders a completion message, not a step", () => {
  const html = renderToStaticMarkup(<StepCard recipe={v60} state={st({ finished: true })} />);
  expect(html.toLowerCase()).toContain("complete");
  expect(html).not.toContain("Step ");
});

test("pre-cue state renders a get-ready message", () => {
  const html = renderToStaticMarkup(<StepCard recipe={v60} state={st({ currentIndex: null })} />);
  expect(html.toLowerCase()).toContain("get ready");
});

test("units config formats the timer numbers like the cards", () => {
  const html = renderToStaticMarkup(<StepCard recipe={v60} state={st({ currentIndex: 0 })} config={{ units: "imperial" }} />);
