  ...
  const html = renderToStaticMarkup(
    <BeanCard
      bean={beanOf({
        ...
        roaster: "R",
        origin: {
          items: [{ country: "ET", process: ["carbonic_maceration"] }],
        },
...
test("the roaster line links the roaster's own page, and the bag's otherwise", () => {
  ...
  expect(card({ name: "Onyx", url: "https://onyx.example" })).toContain(
    'href="https://onyx.example"',
  );
  expect(card({ name: "Onyx", url: "https://onyx.example" })).not.toContain(
  ...
  expect(card({ name: "Onyx", url: "javascript:alert(1)" })).toContain(
...
test("name falls back to Coffee; unsafe url renders no anchor", () => {
  const html = renderToStaticMarkup(
    <BeanCard bean={beanOf({ roaster: "R", url: "javascript:alert(1)" })} />,
  );
...
test("empty-string name falls back to Coffee", () => {
  expect(
    renderToStaticMarkup(
      <BeanCard bean={beanOf({ name: "", roaster: "R" })} />,
    ),
  ).toContain(">Coffee</h3>");
});
  ...
  const html = renderToStaticMarkup(
    <BeanCard
      bean={beanOf({
        ...
        roaster: "R",
        origin: {
          items: [{ producers: [{ name: "Someone", role: "cup-taster" }] }],
        },
      })}
    />,
  );
