...
function Probe({ finishS, steps: stepsProp }: { finishS: number | null; steps?: NormalizedStep[] }) {
  const b = useBrewAlong(stepsProp ?? steps, finishS);
  return (
    <>
      <div data-current={String(b.state.currentIndex)} data-finished={String(b.state.finished)} data-elapsed={Math.floor(b.elapsedS)}
           onClick={() => b.tapDone()} />
      <button data-action="pause" onClick={() => b.pause()}>pause</button>
      <button data-action="resume" onClick={() => b.resume()}>resume</button>
    </>
  );
}
let container: HTMLDivElement; let root: Root;
beforeEach(() => { vi.useFakeTimers({ toFake: ["setTimeout", "clearTimeout", "performance", "Date"] }); container = document.createElement("div"); document.body.appendChild(container); root = createRoot(container); });
...
test("auto-starts; active step advances as the clock passes each cue", () => {
  ...
  act(() => { vi.advanceTimersByTime(60_000); });          // past 90s cue
...
test("finishes at finishS", () => {
  ...
  expect(container.querySelector("div")!.getAttribute("data-finished")).toBe("true");
});
test("paused time is excluded from elapsed", () => {
  act(() => root.render(<Probe finishS={120} />));
  const el = () => container.querySelector("div")!;
  const btn = (action: string) => container.querySelector(`[data-action="${action}"]`) as HTMLButtonElement;
  act(() => { vi.advanceTimersByTime(10_000); });
  act(() => { btn("pause").click(); });
  act(() => { vi.advanceTimersByTime(20_000); });          // paused — must NOT count
  act(() => { btn("resume").click(); });
...
test("autoStart=false holds idle at 0 until start() is called", () => {
  function IdleProbe() {
    ...
    return <div data-elapsed={Math.floor(b.elapsedS)} data-running={String(b.running)} onClick={() => b.start()} />;
  }
...
test("an unstable steps identity does not restart the clock", async () => {
  const DOC = {
    coffeejson: "1.0",
    recipes: [{
      title: "T", method: "pour_over", finish_s: 300,
      coffee: { value: 20, unit: "gram" },
      steps: [{ at_s: 0, kind: "bloom" }, { at_s: 30, kind: "pour" }, { at_s: 90, kind: "pour" }],
    }],
  };
  let renders = 0;
  function Unstable() {
  ...
  try { localRoot.unmount(); } catch { /* a root mid-loop may refuse to unmount cleanly */ }
  el.remove();
