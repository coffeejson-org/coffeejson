...
const untimed = recipeOf({
  title: "Untimed", method: "pour_over",
  steps: [
    { at_s: 0, to_water: { value: 60, unit: "gram" }, instruction: "Bloom" },
    { instruction: "Swirl the bed" },
    { at_s: 90, to_water: { value: 180, unit: "gram" }, instruction: "Second pour" },
  ],
  finish_s: 150,
});
...
test("a step with no time renders the placeholder, not an empty cell", () => {
  const html = renderToStaticMarkup(<StepList recipe={untimed} state={st({ currentIndex: 1, doneIndexes: [0] })} elapsedS={30} />);
...
test("one row per step plus a finish row; index + time render", () => {
  const html = renderToStaticMarkup(<StepList recipe={v60} state={st({ currentIndex: 1, doneIndexes: [0] })} elapsedS={45} />);
...
test("derived 'Pour to {target}' label, overridden by a recipe's own step text", () => {
  const html = renderToStaticMarkup(<StepList recipe={bare} state={st({ currentIndex: 0 })} elapsedS={0} />);
...
test("first targeted row shows its pour unsigned; later rows are +signed", () => {
  const html = renderToStaticMarkup(<StepList recipe={v60} state={st({ currentIndex: 1 })} elapsedS={45} />);
...
test("the current row is aria-current + --current", () => {
  const html = renderToStaticMarkup(<StepList recipe={v60} state={st({ currentIndex: 2, doneIndexes: [0, 1] })} elapsedS={90} />);
  expect(html).toContain("cj-steplist-row--current");
  expect(html).toContain('aria-current="step"');
});

test("finish row shows 'Finish' + the finishS clock", () => {
  const html = renderToStaticMarkup(<StepList recipe={v60} state={st({ finished: true })} elapsedS={150} />);
...
test("--cj-fill: 100% for a passed segment, 50% partial, 0% future (elapsedS=22.5)", () => {
  const html = renderToStaticMarkup(<StepList recipe={v60} state={st({ currentIndex: 1, doneIndexes: [0] })} elapsedS={22.5} />);
...
test("units config reaches the step list, matching the step card", () => {
  const html = renderToStaticMarkup(
    <StepList recipe={v60} state={st({ currentIndex: 1 })} elapsedS={45} config={{ units: "imperial" }} />,
  );
