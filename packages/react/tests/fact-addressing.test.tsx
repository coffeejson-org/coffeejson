...
const beanOf = (b: object) => normalize({ coffeejson: "1.0", beans: [b] }).beans[0]!;
...
test("recipe fact rows carry stable data-cj-fact ids", () => {
  const html = renderToStaticMarkup(<RecipeCard recipe={recipeOf({
    ...
    water_temp: { value: 93, unit: "celsius" }, grind: { setting: "18" }, finish_s: 180,
  })} />);
...
test("espresso-specific fact rows carry data-cj-fact ids", () => {
  const html = renderToStaticMarkup(<RecipeCard recipe={recipeOf({
    title: "Shot", method: "espresso",
    coffee: { value: 19, unit: "gram" }, yield: { value: 47, unit: "gram" },
    water_temp: { value: 93, unit: "celsius" }, pressure: { value: 9, unit: "bar" },
    preinfusion_s: 5,
  })} />);
...
test("bean fact rows carry data-cj-fact ids, including the structural roaster/origin/notes rows", () => {
  const html = renderToStaticMarkup(<BeanCard bean={beanOf({
    name: "Las Brisas", roaster: { name: "Onyx" },
    origin: { items: [{ name: "Finca X", country: "CO" }] },
    process: ["washed"], varietals: ["Caturra"], roast_level: "light_medium",
