...
const deltas = recipeOf({
  title: "V60",
  steps: [
    { kind: "pour", at_s: 0, to_water: { value: 60, unit: "gram" }, instruction: "Bloom" },
    { kind: "stir", at_s: 30, instruction: "Stir gently" },
    { kind: "pour", at_s: 60, to_water: { value: 120, unit: "gram" }, instruction: "Pour to 120g" },
  ],
});
...
function stepRow(html: string, index: number): string {
  const m = new RegExp(`<li class="cj-step[^"]*" data-step="${index}">.*?</li>`, "s").exec(html);
...
test("multi-pour recipe: the initial-fill row shows its own amount unsigned (no +, delta before target)", () => {
  ...
  expect(row0).toContain('<span class="cj-step-delta">60 g</span> <strong class="cj-step-target">60 g</strong>');
...
test("multi-pour recipe: a later pour is a signed +increment, delta before target", () => {
  const html = renderToStaticMarkup(<RecipeCard recipe={deltas} />);
  expect(stepRow(html, 2))
    .toContain('<span class="cj-step-delta">+60 g</span> <strong class="cj-step-target">120 g</strong>');
});
...
test("single-target recipe renders the plain nested-total markup, no Pour column", () => {
  ...
  expect(stepRow(html, 0)).toContain('<strong class="cj-step-target"> 50 g</strong>');
...
test("mid-recipe underivable delta gets the — placeholder while the column still exists", () => {
  ...
  const dip = recipeOf({
    ...
    steps: [
      { kind: "pour", at_s: 0, to_water: { value: 60, unit: "gram" }, instruction: "Fill" },
      { kind: "pour", at_s: 20, to_water: { value: 120, unit: "gram" }, instruction: "Pour" },
      { kind: "pour", at_s: 40, to_water: { value: 90, unit: "gram" }, instruction: "Oops" },
      { kind: "pour", at_s: 60, to_water: { value: 200, unit: "gram" }, instruction: "Recover" },
    ],
  });
  const html = renderToStaticMarkup(<RecipeCard recipe={dip} />);
  expect(stepRow(html, 0))
    .toContain('<span class="cj-step-delta">60 g</span> <strong class="cj-step-target">60 g</strong>');
  expect(stepRow(html, 1)).toContain('<span class="cj-step-delta">+60 g</span>');
  expect(stepRow(html, 2))
    .toContain('<span class="cj-step-delta cj-step-delta--none">—</span> <strong class="cj-step-target">90 g</strong>');
  expect(stepRow(html, 3)).toContain('<span class="cj-step-delta">+110 g</span>');
});
...
test("out-of-vocabulary units: an undisplayable delta renders the placeholder, never a lone +", () => {
  ...
  const liters = recipeOf({
    title: "Cold brew jug",
    steps: [
      { kind: "pour", at_s: 0, to_water: { value: 60, unit: "liter" }, instruction: "Fill A" },
      { kind: "pour", at_s: 60, to_water: { value: 120, unit: "liter" }, instruction: "Fill B" },
      { kind: "pour", at_s: 120, to_water: { value: 180, unit: "liter" }, instruction: "Fill C" },
    ],
  });
  ...
  expect(stepRow(html, 1)).toContain('<span class="cj-step-delta cj-step-delta--none">—</span>');
});

test("active-step highlight is unaffected by the columnar rendering", () => {
  const html = renderToStaticMarkup(<RecipeCard recipe={deltas} activeStepIndex={2} />);
...
const volumeSchedule = recipeOf({
  title: "Volume pour-over", method: "pour_over",
  coffee: { value: 20, unit: "gram" },
  water: { value: 320, unit: "milliliter" },
  steps: [
    { kind: "pour", at_s: 0, to_water: { value: 60, unit: "milliliter" }, instruction: "Bloom" },
    { kind: "pour", at_s: 45, to_water: { value: 320, unit: "milliliter" }, instruction: "Pour to 320" },
  ],
});
...
test("a volume pour schedule renders unsigned fill then signed increment", () => {
  const html = renderToStaticMarkup(<RecipeCard recipe={volumeSchedule} />);
  expect(stepRow(html, 0))
    .toContain('<span class="cj-step-delta">60 mL</span> <strong class="cj-step-target">60 mL</strong>');
  expect(stepRow(html, 1))
    .toContain('<span class="cj-step-delta">+260 mL</span> <strong class="cj-step-target">320 mL</strong>');
...
test("a volume measurement is not converted by any unit system", () => {
  for (const units of ["metric", "imperial"] as const) {
    const html = renderToStaticMarkup(<RecipeCard recipe={volumeSchedule} config={{ units }} />);
