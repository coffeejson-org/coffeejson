...
test("show hides sections", () => {
  const html = renderToStaticMarkup(
    <RecipeCard
      recipe={espresso}
      config={{ show: { grind: false, espresso: false } }}
    />,
  );
  expect(html).not.toContain(">Grind<");
  expect(html).not.toContain(">Pressure<");
});
test("units convert measurements", () => {
  const html = renderToStaticMarkup(
    <RecipeCard recipe={espresso} config={{ units: "imperial" }} />,
  );
...
test("classNames append to the frozen cj-* class, never replace it", () => {
  const html = renderToStaticMarkup(
    <CoffeeJSONView
      doc={{ coffeejson: "1.0", recipes: [{ title: "t" }] }}
      config={{ classNames: { card: "brand-card" } }}
    />,
  );
...
test("labels.brew re-words every brew control and caption", () => {
  const config = {
    labels: {
      brew: {
        pause: "Anhalten",
        reset: "Neu starten",
        complete: "Fertig gebrüht",
      },
    },
  };
  const running = renderToStaticMarkup(
    <BrewAlong recipe={v60} brew={brewOf({ running: true })} config={config} />,
  );
  ...
  const done = renderToStaticMarkup(
    <BrewAlong
      recipe={v60}
      brew={brewOf({ running: false, state: st({ finished: true }) })}
      config={config}
    />,
  );
...
test("resolveConfig returns one object per config identity", () => {
  ...
  expect(resolveConfig({ units: "metric" })).not.toBe(
    resolveConfig({ units: "metric" }),
  );
...
test("a malformed locale falls back instead of throwing", () => {
  ...
  expect(() =>
    fmt({ value: 1, unit: "gram" }, resolveConfig({ locale: "zz" })),
  ).not.toThrow();
});
...
test("a malformed locale does not break a card render", () => {
  const html = renderToStaticMarkup(
    <RecipeCard recipe={espresso} config={{ locale: "en_US" }} />,
  );
