...
test("POUR n / N mid-brew (accent n)", () => {
  const html = renderToStaticMarkup(<PourCounter recipe={v60} state={st({ currentIndex: 1 })} />);
...
test("Done when finished", () => {
  const html = renderToStaticMarkup(<PourCounter recipe={v60} state={st({ finished: true })} />);
...
test("stir and drain steps are not pours", () => {
  const html = renderToStaticMarkup(<PourCounter recipe={mixed} state={st({ currentIndex: 3 })} />);
  expect(html).toContain('cj-pourcounter-n">3<');
  expect(html).toContain("/ 3");
});

test("a recipe that never pours renders nothing", () => {
  const espresso = recipeOf({ title: "Shot", steps: [{ at_s: 0, kind: "press", instruction: "Pull" }] });
  expect(renderToStaticMarkup(<PourCounter recipe={espresso} state={st({})} />)).toBe("");
});
