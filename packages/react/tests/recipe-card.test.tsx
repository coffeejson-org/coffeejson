...
test("a card never renders provenance — `generator` is a document fact", () => {
  ...
  const html = renderToStaticMarkup(
    <RecipeCard
      recipe={recipeOf({
        title: "t",
        source: { app: "OldApp", url: "https://old.example" },
      })}
    />,
  );
...
test("activeStepIndex adds cj-step--active to exactly that step", () => {
  const html = renderToStaticMarkup(
    <RecipeCard recipe={filter} activeStepIndex={1} />,
  );
...
test("recommended badge renders only when set", () => {
  expect(
    renderToStaticMarkup(
      <RecipeCard recipe={recipeOf({ title: "t", recommended: true })} />,
    ),
  ).toContain(">Recommended<");
  expect(renderToStaticMarkup(<RecipeCard recipe={filter} />)).not.toContain(
    "Recommended",
  );
});
...
test("React escaping neutralizes markup in payload content", () => {
  const html = renderToStaticMarkup(
    <RecipeCard recipe={recipeOf({ title: "<script>alert(1)</script>" })} />,
  );
