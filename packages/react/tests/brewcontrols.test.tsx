...
const st = (over: Partial<TimerState>): TimerState =>
  ({ currentIndex: 1, awaitingTap: false, nextTimedIndex: null, doneIndexes: [], finished: false, ...over });
const brewOf = (over: Partial<BrewAlongState>): BrewAlongState => ({
  elapsedS: 0, state: st({}), running: true,
  start() {}, pause() {}, resume() {}, reset() {}, tapDone() {}, ...over,
});

test("Pause label while running, Resume while paused", () => {
  expect(renderToStaticMarkup(<BrewControls brew={brewOf({ running: true })} />)).toContain('aria-label="Pause"');
  expect(renderToStaticMarkup(<BrewControls brew={brewOf({ running: false })} />)).toContain('aria-label="Resume"');
});
...
test("Pause/Resume hidden when finished; Reset remains", () => {
  const html = renderToStaticMarkup(<BrewControls brew={brewOf({ state: st({ finished: true }) })} />);
...
test("tapDone button only when awaitingTap", () => {
  expect(renderToStaticMarkup(<BrewControls brew={brewOf({})} />)).not.toContain('aria-label="Done, next step"');
  expect(renderToStaticMarkup(<BrewControls brew={brewOf({ state: st({ awaitingTap: true }) })} />)).toContain('aria-label="Done, next step"');
});
...
test("the text variant renders labels as button text, with no aria-label", () => {
  const html = renderToStaticMarkup(<BrewControls brew={brewOf({ running: true })} variant="text" />);
...
test("both variants render the same buttons under the same conditions", () => {
  for (const variant of ["icons", "text"] as const) {
    const awaiting = renderToStaticMarkup(
      <BrewControls brew={brewOf({ state: st({ awaitingTap: true }) })} variant={variant} />);
    const finished = renderToStaticMarkup(
      <BrewControls brew={brewOf({ state: st({ finished: true }) })} variant={variant} />);
...
test("labels.brew re-words the controls in both variants", () => {
  const config = { labels: { brew: { pause: "Anhalten", reset: "Neu starten" } } };
  const text = renderToStaticMarkup(
    <BrewControls brew={brewOf({ running: true })} variant="text" config={config} />);
  expect(text).toContain(">Anhalten<");
  expect(text).toContain(">Neu starten<");
  const icons = renderToStaticMarkup(
    <BrewControls brew={brewOf({ running: true })} variant="icons" config={config} />);
