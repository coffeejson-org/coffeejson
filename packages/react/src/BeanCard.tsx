...
import { originLine, processLine, safeUrl, vocabularyLabel } from "@coffeejson/core";
    ...
    <article className={`${cx("card", rc)} ${cx("beanCard", rc)}`}>
      ...
      {roasterText ? (
        <FactShell id="roaster" label={rc.labels.facts.roaster} rc={rc}>
          {href ? <a href={href} rel="noopener">{roasterText}</a> : roasterText}
        </FactShell>
      ) : null}
      {b.originItems.length > 0 ? (
        <FactShell id="origin" label={rc.labels.facts.origin} rc={rc}>
          {b.originItems.map((it, i) => (
            <span className={cx("originItem", rc)} key={i}>{originText(it, rc)}</span>
          ))}
        </FactShell>
      ) : null}
      ...
      <Fact id="process" label={rc.labels.facts.process} value={[processLine(b.process, rc.labels), b.dryingMethod ?? ""].filter(Boolean).join(" · ")} rc={rc} />
      <Fact id="varietals" label={rc.labels.facts.varietals} value={b.varietals.join(", ")} rc={rc} />
      <Fact id="roast" label={rc.labels.facts.roast} value={roastLine} rc={rc} />
      {b.roasterNotes.length > 0 ? (
        <FactShell id="roasterNotes" label={rc.labels.facts.roasterNotes} rc={rc}>
          <em className={cx("notes", rc)}>{b.roasterNotes.join(", ")}</em>
        </FactShell>
      ) : null}
      {b.description ? <p className={cx("description", rc)}>{b.description}</p> : null}
    </article>
  );
}
