...
export function PourCounter({ recipe, state, config }: PourCounterProps) {
  ...
  if (state.finished) return <p className={`${cx("pourCounter", rc)} cj-pourcounter--done`}>{rc.labels.brew.done}</p>;
  ...
  const pourAt = recipe.steps.flatMap((s, i) => (s.toWater !== null ? [i] : []));
  ...
  return (
    <p className={cx("pourCounter", rc)}>
      {rc.labels.brew.pourCounter} <span className={cx("pourCounterN", rc)}>{n}</span> / {pourAt.length}
    </p>
  );
}
