 ...
 */
export function CoffeeJSONView({ doc, renderEmpty, config }: CoffeeJSONViewProps) {
  ...
  return (
    <div className={cx("view", rc)}>
      {rc.show.bean ? n.beans.map((b, i) => <BeanCard bean={b} config={config} key={`b${i}`} />) : null}
      {n.recipes.map((r, i) => <RecipeCard recipe={r} config={config} key={`r${i}`} />)}
      {rc.show.generator && g ? (
        <p className={cx("generator", rc)}>
          {rc.labels.generatedBy}: {gHref ? <a href={gHref} rel="noopener">{gText}</a> : gText}
        </p>
      ) : null}
    </div>
  );
