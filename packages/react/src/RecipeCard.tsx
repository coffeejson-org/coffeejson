...
function StepItem({
    ...
    return (
      <li className={liClass} data-step={index}>
        ...
        <span className={cx("stepBody", rc)}>
          ...
          {step.text}
          {step.toWater ? (
            <strong className={cx("stepTarget", rc)}>
              {" "}
              {fmt(step.toWater, rc)}
            </strong>
          ) : null}
  ...
  if (step.toWater) {
    const target = (
      <strong className={cx("stepTarget", rc)}>{fmt(step.toWater, rc)}</strong>
    );
    // With no amount to state, the placeholder — collapsing misaligns the grid.
    const delta = pour ? (
      <span className={cx("stepDelta", rc)}>{pour}</span>
    ) : (
      <span className={`${cx("stepDelta", rc)} ${cx("stepDeltaNone", rc)}`}>
        —
      </span>
    );
    water = (
...
export function RecipeCard({
  ...
  const espressoDose = [fmt(r.coffee, rc), fmt(r.yield, rc)]
    .filter(Boolean)
    .join(" → ");
  const grind = r.grind
    ? [
  ...
  const filterText = r.filter
    ? [
        vocabularyLabel(rc.labels.filterMaterials, r.filter.material),
        r.filter.label,
      ]
        .filter(Boolean)
        .join(" · ")
  ...
  const additionsText = r.additions
    .map((a) =>
      [rc.labels.additionKinds[a.kind], fmt(a.amount, rc)]
        .filter(Boolean)
        .join(" · "),
    )
    .filter(Boolean)
    .join(", ");
  const subtitle = rc.show.gear
    ? [methodText, r.brewerLabel].filter(Boolean).join(" · ")
    : "";
  ...
  const hasIncrement = r.steps.some(
    (s, i) => s.pourDelta !== null && i !== firstTargetIndex,
  );
  return (
    <article className={`${cx("card", rc)} ${cx("recipeCard", rc)}`}>
      ...
      {r.isEspresso ? (
        ...
        <>
          <Fact
            id="coffee"
            label={rc.labels.facts.coffee}
            value={fmt(r.coffee, rc)}
            rc={rc}
          />
          <Fact
            id="water"
            label={rc.labels.facts.water}
            value={fmt(r.water, rc)}
            rc={rc}
          />
        </>
      )}
      <Fact
        id="ratio"
        label={rc.labels.facts.ratio}
        value={formatRatio(r.ratio)}
        rc={rc}
      />
      <Fact
        id="waterTemp"
        label={rc.labels.facts.waterTemp}
        value={fmt(r.waterTemp, rc)}
        rc={rc}
      />
      {r.additions.length > 0 ? (
        <Fact
          id="additions"
          label={rc.labels.facts.additions}
          value={additionsText}
          rc={rc}
        />
      ) : null}
      {rc.show.grind ? (
        <Fact id="grind" label={rc.labels.facts.grind} value={grind} rc={rc} />
      ) : null}
      {rc.show.gear ? (
        <Fact
          id="filter"
          label={rc.labels.facts.filter}
          value={filterText}
          rc={rc}
        />
      ) : null}
      {showEspressoBlock ? (
        <>
          <Fact
            id="pressure"
            label={rc.labels.facts.pressure}
            value={fmt(r.pressure, rc)}
            rc={rc}
          />
          <Fact
            id="preinfusion"
            ...
            value={
              r.preinfusionS !== null
                ? `${r.preinfusionS} ${rc.labels.brew.seconds}`
                : ""
            }
            rc={rc}
          />
