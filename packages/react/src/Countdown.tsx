...
export function Countdown({ recipe, state, elapsedS, target = "next", format = "clock", config }: CountdownProps) {
  const rc = resolveConfig(config);
  const label = target === "finish" ? rc.labels.brew.finishIn : rc.labels.brew.nextPourIn;
  const targetS = target === "finish"
    ? recipe.finishS
    : state.nextTimedIndex !== null ? recipe.steps[state.nextTimedIndex]!.atS : null;

  if (state.finished || targetS === null) {
    return (
      <div className={`${cx("countdown", rc)} cj-countdown--idle`}>
        <span className={cx("countdownLabel", rc)}>{state.finished ? rc.labels.brew.done : label}</span>
  ...
  const value = remaining === 0 ? rc.labels.brew.now : format === "seconds" ? `${remaining}s` : fmtClock(remaining);
  const pour = target === "next" && state.nextTimedIndex !== null
