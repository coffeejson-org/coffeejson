...
const PlayIcon = () => (
  <svg viewBox="0 0 24 24" width="24" height="24" aria-hidden="true" fill="currentColor"><path d="M8 5v14l11-7z" /></svg>
);
const ResetIcon = () => (
...
const NextIcon = () => (
  <svg viewBox="0 0 24 24" width="24" height="24" aria-hidden="true" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6 9 17l-5-5" /></svg>
);
...
export function BrewControls({ brew, variant = "icons", config }: BrewControlsProps) {
  const rc = resolveConfig(config);
  const { running, state } = brew;
  const control = (label: string, icon: ReactNode, onClick: () => void, className: string) => (
    <button
      type="button"
      className={className}
  ...
  return (
    <div className={cx("brewControls", rc)}>
      {!state.finished
        ...
        : null}
      {state.awaitingTap
        ? control(rc.labels.brew.next, <NextIcon />, brew.tapDone,
            `${cx("brewBtn", rc)} ${cx("brewBtnPrimary", rc)}`)
        : null}
      {control(rc.labels.brew.reset, <ResetIcon />, brew.reset,
