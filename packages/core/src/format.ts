...
const UNIT: Record<string, string> = Object.assign(
  Object.create(null),
  UNIT_SYMBOLS,
);

/** `format` renders each magnitude — pass a locale-aware one to localize digits. */
export function fmtMeasurement(
  m?: Measurement | null,
...
function fmtSpan(m: Measurement, format: (n: number) => string): string {
  ...
  if (m.min !== undefined && m.max !== undefined)
    return `${format(m.min)}–${format(m.max)}`;
...
export function formatRatio(ratio: number | null): string {
  ...
  const n = Number.isInteger(ratio)
    ? String(ratio)
...
export function gearLabel(
  ...
  const variant =
    typeof o["variant"] === "string" && o["variant"] !== ""
      ? o["variant"]
      : null;
...
export const processLine = (
  ...
  tokens
    .map((t) => vocabularyLabel(labels.processes, t))
    .filter(Boolean)
    .join(" · ");
 ...
 */
export const producerLine = (
  parties: readonly NormalizedParty[],
  labels: LabelSet,
): string =>
  parties
    .map((p) => {
...
export const originLine = (
  ...
  [
    it.name,
    [it.region, it.country !== null ? regionLabel(it.country) : null]
      .filter(Boolean)
      .join(", "),
    ...
    it.varietals.length > 0
      ? `${labels.facts.varietals} ${it.varietals.join(", ")}`
      : "",
