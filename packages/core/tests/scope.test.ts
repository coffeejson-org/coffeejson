...
const beanIds = (d: DecodedDocument | null) =>
  normalize(d).beans.map((b) => b.id);
const beanNames = (d: DecodedDocument | null) =>
  normalize(d).beans.map((b) => b.name);
const recipeTitles = (d: DecodedDocument | null) =>
  normalize(d).recipes.map((r) => r.title);
const tastingIds = (d: DecodedDocument | null) =>
  normalize(d).tastings.map((t) => t.id);
...
const bagToBrew = {
  ...
  recipes: [
    recipe("Dwell"),
    recipe("French Press", { method: "french_press" }),
  ],
};
...
describe("scopeToRecipe", () => {
  ...
  it("omits beans entirely when a bean_ref resolves to nothing", () => {
    const doc = {
      ...multiBean,
      recipes: [recipe("Dangling", { bean_ref: "nope" })],
    };
  ...
  it("omits beans on a document that carries none", () => {
    const out = scopeToRecipe(
      { coffeejson: "1.0", recipes: [recipe("Alone")] },
      0,
    )!;
    expect("beans" in out).toBe(false);
  });
  ...
  it("preserves members it does not recognize", () => {
    ...
    const out = scopeToRecipe(withExt, 0)! as unknown as Record<
      string,
      unknown
    >;
...
describe("scopeToBean", () => {
  ...
  it("returns null when there is no bean at that index", () => {
    ...
    expect(
      scopeToBean({ coffeejson: "1.0", recipes: [recipe("x")] }, 0),
    ).toBeNull();
  });
  ...
  it("preserves unknown members", () => {
    ...
    expect(
      (scopeToBean(withExt, 0)! as unknown as Record<string, unknown>)["ext"],
    ).toEqual({ keep: true });
  });
...
describe("association rule", () => {
  ...
  it("an unresolved bean_ref leaves the recipe unlinked in every reader", () => {
    const doc = {
      coffeejson: "1.0",
      beans: [bean("b1", "One")],
      recipes: [recipe("R", { bean_ref: "nope" })],
    };
  ...
  it("a single co-located bean is the recipe's bean in every reader", () => {
    ...
    expect((scopeToRecipe(doc, 0) as { beans?: unknown[] }).beans).toHaveLength(
      1,
    );
    expect(normalize(doc).recipes[0]!.bean?.name).toBe("Sermon");
    expect(recipeJsonLd(doc, 0)!["recipeIngredient"]).toEqual([
      "20 g coffee — Sermon (R)",
      "300 g water",
    ]);
  });
...
describe("projecting tastings", () => {
  ...
  it("leaves no tasting pointing at a recipe the projection dropped", () => {
    for (const i of [0, 1]) {
      const out = scopeToRecipe(library, i)! as unknown as Record<
        string,
        unknown
      >;
      const ids = new Set(
        (out["recipes"] as { id: string }[]).map((r) => r.id),
      );
  ...
  it("omits the member entirely rather than emitting an empty array", () => {
    const out = scopeToRecipe(
      { ...library, tastings: [{ recipe_ref: "nowhere" }] },
      0,
    )!;
  ...
  it("a tasting follows its own bean_ref, not the recipe's", () => {
    const swap = {
      ...library,
      tastings: [
        { id: "swapped", recipe_ref: "r1", bean_ref: "b2", rating: 4 },
      ],
    };
...
describe("the corpus, projected", () => {
  ...
  const withTastings = corpus.filter(([, d]) =>
    Array.isArray((d as { tastings?: unknown[] }).tastings),
  );
  ...
  it("never projects a tasting whose recipe_ref the projection dropped", () => {
    for (const [name, doc] of corpus)
      for (let i = 0; i < (doc.recipes ?? []).length; i++) {
        const out = scopeToRecipe(doc, i)! as unknown as Record<
          string,
          unknown
        >;
        const ids = new Set(
        ...
        for (const t of (out["tastings"] ?? []) as { recipe_ref?: string }[])
          expect(
            t.recipe_ref === undefined || ids.has(t.recipe_ref),
            `${name} #${i}`,
          ).toBe(true);
      }
  });
  ...
  it("never projects a tasting of another bag into a bean lens", () => {
    for (const [name, doc] of corpus)
      for (let i = 0; i < (doc.beans ?? []).length; i++) {
        ...
        const ids = new Set(
          (out["beans"] as { id?: string }[]).map((b) => b.id),
        );
        for (const t of (out["tastings"] ?? []) as { bean_ref?: string }[])
          expect(
            t.bean_ref === undefined || ids.has(t.bean_ref),
            `${name} #${i}`,
          ).toBe(true);
      }
  });
...
describe("beans a projected tasting names", () => {
  ...
  it("keeps the source's bean order rather than appending", () => {
    const reversed = {
      ...swap,
      beans: [bean("b2", "Drinker's"), bean("b1", "Recipe's")],
    };
...
describe("member order", () => {
  it("leaves a generator written after the collections where the document put it", () => {
    ...
    expect(Object.keys(scopeToRecipe(trailing, 0)!)).toEqual([
      "coffeejson",
      "beans",
      "recipes",
      "generator",
    ]);
    expect(encodePayload(scopeToRecipe(trailing, 0)!)).toBe(
      encodePayload(trailing),
    );
  });
  ...
  it("keeps an unknown member in its original position", () => {
    ...
    expect(Object.keys(scopeToRecipe(withExt, 0)!)).toEqual([
      "coffeejson",
      "ext",
      "recipes",
    ]);
