...
describe("shape", () => {
  test("non-object inputs yield an empty doc", () => {
    for (const v of [null, undefined, 42, "x", true, [1, 2]])
      expect(normalize(v)).toEqual({ beans: [], recipes: [], tastings: [], generator: null });
  });
...
describe("generator (a document-level fact, never a recipe one)", () => {
  ...
  test("projects onto the document", () => {
    expect(doc({ name: "ExampleBrewApp", version: "2.3.0", url: "https://example.com/brewapp" }).generator)
      .toEqual({ name: "ExampleBrewApp", version: "2.3.0", url: "https://example.com/brewapp" });
  });
  test("absent reads as null", () => {
    expect(normalize({ coffeejson: "1.0", recipes: [{ title: "t" }] }).generator).toBeNull();
  });
  test("name is the whole identity — a nameless generator reads as absent", () => {
    ...
    expect(doc({ version: "2.3.0", url: "https://example.com/brewapp" }).generator).toBeNull();
  ...
  test("optional members drop to null individually", () => {
    expect(doc({ name: "ExampleBrewApp" }).generator).toEqual({ name: "ExampleBrewApp", version: null, url: null });
  });
  test("a recipe-level `source` is NOT read — provenance lives on the envelope", () => {
    const n = normalize({
      coffeejson: "1.0",
      recipes: [{ title: "t", source: { app: "OldApp", url: "https://old.example" } }],
    });
...
describe("ratio derivation (from production ratioText semantics)", () => {
  const r = (extra: object) =>
    normalize({ coffeejson: "1.0", recipes: [{ title: "t", ...extra }] }).recipes[0]!;
  test("filter: water/coffee wins, else the explicit ratio", () => {
    expect(r({ coffee: { value: 20, unit: "gram" }, water: { value: 300, unit: "gram" } }).ratio).toBe(15);
    expect(r({ coffee: { value: 20, unit: "gram" }, water: { value: 300, unit: "gram" }, ratio: 17 }).ratio).toBe(15);
    expect(r({ coffee: { value: 20, unit: "gram" }, ratio: 17 }).ratio).toBe(17);
  });
  test("espresso: yield/coffee; explicit ratio ignored", () => {
    ...
    const e = r({ basis: "yield", method: "espresso", coffee: { value: 19, unit: "gram" }, yield: { value: 47, unit: "gram" }, ratio: 99 });
  ...
  test("guards: non-number ratio dropped; zero/absent coffee → null", () => {
    expect(r({ coffee: { value: 20, unit: "gram" }, water: { value: 300, unit: "gram" }, ratio: "abc" }).ratio).toBe(15);
    expect(r({ coffee: { value: 0, unit: "gram" }, water: { value: 300, unit: "gram" } }).ratio).toBeNull();
    expect(r({ water: { value: 300, unit: "gram" } }).ratio).toBeNull();
  });
  test("non-finite / non-positive derived ratios are dropped to null", () => {
    expect(r({ coffee: { value: 5e-324, unit: "gram" }, water: { value: 1e308, unit: "gram" } }).ratio).toBeNull(); // overflow → Infinity → null
    expect(r({ coffee: { value: 20, unit: "gram" }, water: { value: -300, unit: "gram" } }).ratio).toBeNull();      // negative → null
    expect(r({ coffee: { value: 20, unit: "gram" }, ratio: -5 }).ratio).toBeNull();                                  // negative explicit → null
  });
});
...
describe("associated-bean pairing (from production associatedMember semantics)", () => {
  ...
  test("catalog-with-refs: bean_ref exact match", () => {
    ...
    for (const r of n.recipes)
      if (r.bean) expect(n.beans.some((b) => b.id === r.bean!.id || n.beans.length === 1)).toBe(true);
  });
  test("broken ref: no fallback", () => {
    const n = normalize({
      coffeejson: "1.0",
      beans: [{ id: "a", name: "A" }, { id: "b", name: "B" }],
...
describe("steps and field hygiene", () => {
  test("null/non-object steps dropped; label wins over instruction", () => {
    const n = normalize({ coffeejson: "1.0", recipes: [{ title: "t", steps: [
      null, "junk", { at_s: 0, label: "Bloom", instruction: "ignored", to_water: { value: 50, unit: "gram" } }, { instruction: "Stir" },
    ] }] });
    expect(n.recipes[0]!.steps).toEqual([
      { kind: null, atS: 0, toWater: { value: 50, unit: "gram" }, pourDelta: { value: 50, unit: "gram" }, text: "Bloom" },
  ...
  test("recommended is strict-true only; absent gear labels are empty strings", () => {
    const n = normalize({ coffeejson: "1.0", recipes: [{ title: "t", recommended: "yes", brewer: {} }] });
...
describe("pourDelta derivation (vs. last known cumulative to_water)", () => {
  const withSteps = (steps: object[]) =>
    normalize({ coffeejson: "1.0", recipes: [{ title: "t", steps }] }).recipes[0]!.steps;
  ...
  test("mixed units: previous cumulative converted into the current step's unit", () => {
    ...
    expect(steps[1]!.pourDelta).toEqual({ value: 5 - 100 / 28.349523125, unit: "ounce" });
  });
  ...
  test("decreasing cumulative (120 -> 60): initial fill, then null", () => {
    ...
    expect(steps.map((s) => s.pourDelta)).toEqual([{ value: 120, unit: "gram" }, null]);
  });
  ...
  test("no to_water anywhere (e.g. espresso) yields all-null pourDelta", () => {
    ...
    expect(n.recipes[0]!.steps.map((s) => s.pourDelta)).toEqual([null, null, null]);
  });

  test("first targeted step (single-step recipe): the full initial fill", () => {
    const steps = withSteps([{ at_s: 0, to_water: { value: 50, unit: "gram" } }]);
...
const baseRecipe = {
  title: "Base recipe", method: "pour_over",
  brewer: { id: "b1", brand: "Hario", model: "V60" },
  coffee: { value: 20, unit: "gram" }, water: { value: 300, unit: "gram" },
  yield: { value: 40, unit: "gram" }, ratio: 15,
  water_temp: { value: 93, unit: "celsius" },
  grind: { grinder: { id: "g1", label: "Comandante" }, setting: "18", microns_approx: 700 },
  pressure: { value: 9, unit: "bar" }, preinfusion_s: 5, basket: { id: "bk1", label: "VST" },
  steps: [{ kind: "pour", at_s: 0, to_water: { value: 50, unit: "gram" }, instruction: "Bloom" }],
  finish_s: 180, source: { app: "CoffeeJSON", version: "1.0", url: "https://example.com" },
  bean_ref: "bean1", recommended: true,
};
const baseBean = {
  id: "bean1", name: "Test Coffee",
  roaster: { name: "Test Roaster", url: "https://example-roastery.com", type: "organization" },
  url: "https://example.com/coffee",
  origin: { type: "single-origin", items: [{ name: "Finca X", country: "CO", region: "Huila", process: "Washed", percentage: 100 }] },
  process: "Washed", drying_method: "Patio", varietals: ["Caturra", "Bourbon"],
  roast_level: "medium", roast_agtron: 60, roast_date: "2026-01-01",
...
test("a party roaster normalizes to its display name; a nameless party reads as absent", () => {
  const doc = (roaster: unknown) =>
    ({ coffeejson: "1.0", beans: [{ name: "Bag", roaster }] });
  expect(normalize(doc({ name: "Test Roaster", url: "https://example-roastery.com" })).beans[0]!.roaster)
    .toEqual({ name: "Test Roaster", role: null, url: "https://example-roastery.com", type: null });
  expect(normalize(doc({ url: "https://example-roastery.com" })).beans[0]!.roaster).toBeNull();
});
...
test("a grind projects its qualitative size, an unrecognized token included", () => {
  ...
  const size = (v: unknown) =>
    normalize({ coffeejson: "1.0", recipes: [{ ...baseRecipe, grind: { setting: "18", size: v } }] })
...
test("a party carries its own url and type as authored", () => {
  const roaster = (r: unknown) =>
    normalize({ coffeejson: "1.0", beans: [{ name: "Bag", roaster: r }] }).beans[0]!.roaster;
  // Unfiltered: a link is checked where it becomes an `href`, not in the projection.
  expect(roaster({ name: "R", url: "javascript:alert(1)", type: "organization" }))
    .toEqual({ name: "R", role: null, url: "javascript:alert(1)", type: "organization" });
  expect(roaster({ name: "R", url: 7, type: {} }))
    .toEqual({ name: "R", role: null, url: null, type: null });
});
...
test("field-substitution battery: every recipe/bean/doc field survives", () => {
  for (const field of Object.keys(baseRecipe))
    for (const v of BATTERY)
      expect(() => normalize({ coffeejson: "1.0", beans: [baseBean], recipes: [{ ...baseRecipe, [field]: v }] }),
        `recipe.${field}`).not.toThrow();
  for (const field of Object.keys(baseBean))
    for (const v of BATTERY)
      expect(() => normalize({ coffeejson: "1.0", beans: [{ ...baseBean, [field]: v }], recipes: [baseRecipe] }),
        `bean.${field}`).not.toThrow();
  for (const field of ["coffeejson", "beans", "recipes"])
    for (const v of BATTERY)
      expect(() => normalize({ coffeejson: "1.0", beans: [baseBean], recipes: [baseRecipe], [field]: v }),
        `doc.${field}`).not.toThrow();
});

test("nested-substitution battery: nested sub-objects survive too", () => {
  for (const v of BATTERY) {
    expect(() => normalize({ coffeejson: "1.0", recipes: [{ ...baseRecipe, grind: v, brewer: v, basket: v, steps: v, source: v }] }), `recipe-subobj ${String(v)}`).not.toThrow();
    expect(() => normalize({ coffeejson: "1.0", recipes: [{ ...baseRecipe, grind: { grinder: v, setting: v, microns_approx: v } }] }), `grind.${String(v)}`).not.toThrow();
    expect(() => normalize({ coffeejson: "1.0", recipes: [{ ...baseRecipe, steps: [v, { kind: v, at_s: v, to_water: v, label: v, instruction: v }] }] }), `step.${String(v)}`).not.toThrow();
    expect(() => normalize({ coffeejson: "1.0", beans: [{ ...baseBean, origin: { type: v, items: v } }], recipes: [baseRecipe] }), `origin.${String(v)}`).not.toThrow();
    expect(() => normalize({ coffeejson: "1.0", beans: [{ ...baseBean, origin: { type: "x", items: [v, { name: v, country: v, percentage: v }] } }], recipes: [baseRecipe] }), `origin.item ${String(v)}`).not.toThrow();
  }
});
...
test("normalizes recipe notes and an ice addition", () => {
  const r = normalize({
    coffeejson: "1.0",
    recipes: [{
      title: "Iced 4:6", coffee: { value: 20, unit: "gram" }, water: { value: 150, unit: "gram" },
...
test("a stated filter is projected, material and label", () => {
  const n = normalize({
    coffeejson: "1.0",
    recipes: [{ title: "V60", coffee: { value: 15, unit: "gram" }, water: { value: 250, unit: "gram" },
      filter: { material: "paper", label: "Hario tabbed" } }],
  });
  expect(n.recipes[0]!.filter).toEqual({ material: "paper", label: "Hario tabbed" });
});

test("a filter with no label keeps its material", () => {
  const n = normalize({
    coffeejson: "1.0",
    recipes: [{ title: "Press", coffee: { value: 30, unit: "gram" }, water: { value: 500, unit: "gram" },
...
test("no filter projects null, not a hollow object", () => {
  const n = normalize({ coffeejson: "1.0", recipes: [{ title: "x", coffee: { value: 1, unit: "gram" }, water: { value: 1, unit: "gram" } }] });
...
describe("water derived from a stated ratio", () => {
  test("a dose and a ratio state a total, so one is projected", () => {
    const n = normalize(load("fixtures/valid/recipe-ratio-instead-of-water.json"));
    const r = n.recipes[0]!;
  ...
  test("the dose's unit carries, because a ratio is by mass", () => {
    const n = normalize({
      coffeejson: "1.0",
      recipes: [{ title: "ounces", coffee: { value: 0.7, unit: "ounce" }, ratio: 10 }],
    });
...
describe("tastings", () => {
  const doc = {
    ...
    recipes: [
      ...
      { id: "other", title: "Other", coffee: { value: 15, unit: "gram" }, water: { value: 250, unit: "gram" } },
    ],
    tastings: [{
      id: "monday", recipe_ref: "roasters-v60", bean_ref: "las-brisas", rating: 4,
      perceived: { extraction: -0.2 }, descriptors: ["blackberry", "dark chocolate"],
  ...
  test("a single co-located bean associates a tasting that names none", () => {
    const bagToBrew = { ...doc, beans: [doc.beans[0]], tastings: [{ recipe_ref: "roasters-v60" }] };
    expect(normalize(bagToBrew).tastings[0]!.bean!.name).toBe("Nano Challa");
  });

  test("several beans and no bean_ref associates nothing", () => {
    const t = normalize({ ...doc, tastings: [{ recipe_ref: "roasters-v60" }] }).tastings[0]!;
  ...
  test("no recipe_ref means no recipe, even with one recipe in the document", () => {
    const one = { ...doc, recipes: [doc.recipes[0]], tastings: [{ rating: 5 }] };
  ...
  test("an unresolved recipe_ref leaves the tasting unlinked rather than failing", () => {
    const t = normalize({ ...doc, tastings: [{ recipe_ref: "nope" }] }).tastings[0]!;
    expect(t.recipe).toBeNull();
  });

  test("a perceived or measured object that states nothing reads as absent", () => {
    const t = normalize({ ...doc, tastings: [{ perceived: {}, measured: {} }] }).tastings[0]!;
  ...
  test("junk in the tastings array never throws", () => {
    ...
    expect(normalize({ coffeejson: "1.0", tastings: [1, "x", null, {}] }).tastings).toHaveLength(1);
  });
});
...
describe("derived extraction yield", () => {
  const doc = (tasting: Record<string, unknown>, recipe: Record<string, unknown> = {}) => ({
    coffeejson: "1.0",
    recipes: [{
      id: "r", title: "R", coffee: { value: 18, unit: "gram" },
      water: { value: 300, unit: "gram" }, ...recipe,
    }],
  ...
  test("(beverage mass x tds) / dose, from the recipe's target yield", () => {
    ...
    expect(ey({ measured: { tds: 1.38 } }, { yield: { value: 262, unit: "gram" } }))
  ...
  test("an immersion recipe with no target yield still derives, given a weighed cup", () => {
    ...
    expect(ey({ measured: { tds: 1.31, yield: { value: 431, unit: "gram" } } }))
      .toBeCloseTo(31.3672, 3);
  });

  test("mixed mass units convert before dividing", () => {
    const gramBased = ey({ measured: { tds: 1.4, yield: { value: 250, unit: "gram" } } });
    const ounceBased = ey({ measured: { tds: 1.4, yield: { value: 250 / 28.349523125, unit: "ounce" } } });
  ...
  test("a missing input derives nothing rather than guessing", () => {
    ...
    expect(normalize({ coffeejson: "1.0", tastings: [{ measured: { tds: 1.4 } }] })
      .tastings[0]!.extractionYield).toBeNull();                            // no recipe, so no dose
  });

  test("a windowed beverage mass takes its midpoint, as the espresso ratio does", () => {
    // 33 g midpoint x 9 % / 18 g = 16.5
    expect(ey({ measured: { tds: 9, yield: { min: 32, max: 34, unit: "gram" } } })).toBeCloseTo(16.5, 6);
  });
  ...
  test("junk inputs never throw and never produce a number", () => {
    for (const v of [0, -1, "x", null, Infinity, NaN])
      expect(ey({ measured: { tds: v, yield: { value: 250, unit: "gram" } } })).toBeNull();
  });
});
...
describe("duplicate ids leave every reference to them unresolved", () => {
  test("two beans sharing an id: a bean_ref to it resolves to no bean", () => {
    const n = normalize({
      ...
      recipes: [{ title: "t", bean_ref: "dup", coffee: { value: 15, unit: "gram" }, water: { value: 250, unit: "gram" } }],
  ...
  test("two recipes sharing an id: a recipe_ref to it resolves to no recipe", () => {
    const n = normalize({
      coffeejson: "1.0",
      recipes: [
        { id: "dup", title: "First", coffee: { value: 15, unit: "gram" }, water: { value: 250, unit: "gram" } },
        { id: "dup", title: "Second", coffee: { value: 15, unit: "gram" }, water: { value: 250, unit: "gram" } },
      ],
  ...
  test("two tastings sharing an id: both are carried, and each resolves its own references", () => {
    const n = normalize({
      coffeejson: "1.0",
      beans: [{ id: "a", name: "A" }, { id: "b", name: "B" }],
      recipes: [{ id: "r", title: "t", coffee: { value: 15, unit: "gram" }, water: { value: 250, unit: "gram" } }],
      tastings: [
  ...
  test("one match still resolves, and no match still resolves to nothing", () => {
    const doc = (ref: string) => normalize({
      coffeejson: "1.0",
      beans: [{ id: "a", name: "A" }, { id: "b", name: "B" }],
      recipes: [{ title: "t", bean_ref: ref, coffee: { value: 15, unit: "gram" }, water: { value: 250, unit: "gram" } }],
    });
...
describe("ratio derivation brings both operands to one mass unit", () => {
  ...
  test("a gram dose against an ounce water converts before dividing", () => {
    expect(r({ coffee: { value: 15, unit: "gram" }, water: { value: 8, unit: "ounce" } }).ratio)
      .toBeCloseTo((8 * 28.349523125) / 15, 6);
  });

  test("espresso converts too: a gram dose against an ounce yield", () => {
    expect(r({ basis: "yield", coffee: { value: 18, unit: "gram" }, yield: { value: 1.27, unit: "ounce" } }).ratio)
      .toBeCloseTo((1.27 * 28.349523125) / 18, 6);
  });

  test("one unit is as good as two: ounce over ounce needs no conversion", () => {
    expect(r({ coffee: { value: 0.5, unit: "ounce" }, water: { value: 7.5, unit: "ounce" } }).ratio).toBe(15);
  });

  test("a unit with no mass conversion on either side derives no ratio", () => {
    expect(r({ coffee: { value: 15, unit: "gram" }, water: { value: 0.25, unit: "liter" } }).ratio).toBeNull();
    expect(r({ coffee: { value: 0.015, unit: "liter" }, water: { value: 250, unit: "gram" } }).ratio).toBeNull();
    expect(r({ basis: "yield", coffee: { value: 18, unit: "celsius" }, yield: { value: 36, unit: "gram" } }).ratio).toBeNull();
    expect(r({ coffee: { value: 15, unit: "gram" }, water: { value: 250, unit: "milliliter" } }).ratio).toBeNull();
  });

  test("a window on one side still derives, from its midpoint", () => {
    expect(r({ coffee: { min: 18, max: 20, unit: "gram" }, water: { value: 285, unit: "gram" } }).ratio).toBe(15);
    expect(r({ coffee: { value: 19, unit: "gram" }, water: { min: 9, max: 11, unit: "ounce" } }).ratio)
      .toBeCloseTo((10 * 28.349523125) / 19, 6);
  });

  test("a dose in a unit with no mass conversion derives no water from a ratio", () => {
    expect(r({ coffee: { value: 0.02, unit: "liter" }, ratio: 15 }).water).toBeNull();
  });
});

describe("an origin item projects every fact the schema gives it", () => {
  const item = (it: object) =>
    normalize({ coffeejson: "1.0", beans: [{ name: "b", origin: { items: [it] } }] }).beans[0]!.originItems[0]!;
  ...
  test("producers, altitude, varietals and harvest time all reach the view model", () => {
    expect(item({
      ...
      name: "Finca X", country: "CO", region: "Huila",
      producers: [{ name: "Elias Roa", role: "farm", url: null, type: null },
                  { name: "Coop Huila", role: null, url: null, type: null }],
      altitude: { min: 1700, max: 1900, unit: "meter" },
      varietals: ["Caturra", "Bourbon"],
      process: ["washed"], harvestTime: "Oct–Dec 2024", percentage: 60,
    });
  });

  test("a party with an unrecognized role is carried, role and all", () => {
    expect(item({ producers: [{ name: "Someone", role: "cup-taster" }] }).producers)
      .toEqual([{ name: "Someone", role: "cup-taster", url: null, type: null }]);
  });

  test("a party with no usable name states nothing", () => {
    expect(item({ producers: [{ role: "farm" }, { name: "   " }, "Elias", { name: "Real" }] }).producers)
  ...
  test("an item stating none of them reads as empty, never as holes", () => {
    expect(item({ country: "ET" })).toEqual({
      name: null, country: "ET", region: null, producers: [], altitude: null,
      varietals: [], process: [], harvestTime: null, percentage: null,
    });
  });
...
describe("the measurements are authoritative, the stated ratio a convenience", () => {
  test("a stated ratio its own coffee and water contradict is recomputed", () => {
    const n = normalize(load("fixtures/valid/recipe-ratio-disagrees-with-water.json"));
    expect(n.recipes[0]!.ratio).toBe(15);
  });

  test("a stated ratio stands where the measurements state none", () => {
    const r = (extra: object) =>
      normalize({ coffeejson: "1.0", recipes: [{ title: "t", ratio: 16.7, ...extra }] }).recipes[0]!;
    ...
    expect(r({ coffee: { value: 20, unit: "gram" }, water: { value: 300, unit: "milliliter" } }).ratio).toBe(16.7);
  });
});
...
describe("an unrecognized basis is derived from the quantities present", () => {
  ...
  test("water or ratio present reads as water-basis, whatever the token says", () => {
    expect(r({ basis: "espresso", coffee: dose, water: { value: 270, unit: "gram" } }).isEspresso).toBe(false);
    expect(r({ basis: "espresso", coffee: dose, ratio: 15 }).isEspresso).toBe(false);
    // Even beside a yield: a water-basis recipe MAY also state the beverage out.
    expect(r({ basis: "espresso", coffee: dose, water: { value: 270, unit: "gram" }, yield: { value: 250, unit: "gram" } }).isEspresso).toBe(false);
  });

  test("only a yield reads as yield-basis", () => {
    expect(r({ basis: "espresso", coffee: dose, yield: { value: 36, unit: "gram" } }).isEspresso).toBe(true);
  });

  test("the switch is `basis`, not `method`: a stated basis is never derived over", () => {
    expect(r({ basis: "yield", method: "pour_over", coffee: dose, yield: { value: 36, unit: "gram" } }).isEspresso).toBe(true);
    expect(r({ basis: "water", method: "espresso", coffee: dose, yield: { value: 36, unit: "gram" } }).isEspresso).toBe(false);
    expect(r({ method: "espresso", coffee: dose, water: { value: 270, unit: "gram" } }).isEspresso).toBe(false);
  });
...
describe("calendar dates", () => {
  const roastDateOf = (roast_date: unknown) =>
    normalize({ coffeejson: "1.0", beans: [{ name: "Nano Challa", roast_date }] }).beans[0]!.roastDate;
...
describe("a recipe's provenance", () => {
  const recipeOf = (extra: Record<string, unknown>) =>
    normalize({ coffeejson: "1.0", recipes: [{ title: "V60", ...extra }] }).recipes[0]!;
  test("author, source, description, language and date project", () => {
    const r = recipeOf({
      author: { name: "Example Roastery", type: "organization", role: "roaster",
    ...
    expect(r.author).toEqual({ name: "Example Roastery", role: "roaster",
      url: "https://roastery.example", type: "organization" });
