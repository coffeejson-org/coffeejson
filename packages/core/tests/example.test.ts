...
import {
  decodeScanned, encodePayload, fmtClock, fmtMeasurement, formatRatio, normalize,
} from "../src/index";
...
test("decode a share link and read a recipe", () => {
  const link = `https://coffeejson.org/r?d=${encodePayload({
    ...
    recipes: [{
      ...
      steps: [
        ...
        { kind: "pour", at_s: 45, to_water: { value: 150, unit: "gram" }, instruction: "First pour" },
        { kind: "pour", at_s: 90, to_water: { value: 250, unit: "gram" }, instruction: "Final pour" },
      ],
    }],
  })}`;
  ...
  const lines = [
    ...
    ...recipe.steps.map((s) => `${fmtClock(s.atS!)} ${s.text} → ${fmtMeasurement(s.toWater)}`),
  ];
